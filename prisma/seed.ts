...
async function seedFAQs() {
  ...
  for (const [i, f] of faqs.entries()) {
    await prisma.fAQ.upsert({
      where: { slug: f.slug },
      // Fix audit FAQ 2026-05-31 (axe G4) : les FAQ seed sont du Track A
      // éditorial canonique → tier-1 indexable explicite (le défaut schéma
      // tier-2 les sortait à tort en noindex). `update` promeut aussi les
      // lignes prod existantes au prochain `pnpm db:seed`.
      update: { indexationTier: "tier_1_indexable", isAutoGenerated: false },
      create: {
        slug: f.slug,
        category: f.category,
        questionFr: f.qFr,
