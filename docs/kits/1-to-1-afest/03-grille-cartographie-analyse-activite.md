# Grille de cartographie / analyse de l'activité (AFEST)

**À remplir en ouverture de journée**, avec la personne. C'est le **cœur** du 1-to-1 :
on photographie le **fonctionnement réel** avant toute optimisation.

**Bénéficiaire :** **********\_\_********** **Poste / rôle :** **********\_\_**********
**Date :** ****\_\_\_\_**** **Coach :** **********\_\_**********

---

## 1. Tâches d'une semaine type

| #   | Tâche réelle | Fréquence (j/sem ou /jour) | Temps/occurrence | Outils utilisés | Pénibilité (1-5) | Répétitif ? (O/N) | Valeur ajoutée (1-5) |
| --- | ------------ | -------------------------- | ---------------- | --------------- | ---------------- | ----------------- | -------------------- |
| 1   |              |                            |                  |                 |                  |                   |                      |
| 2   |              |                            |                  |                 |                  |                   |                      |
| 3   |              |                            |                  |                 |                  |                   |                      |
| 4   |              |                            |                  |                 |                  |                   |                      |
| 5   |              |                            |                  |                 |                  |                   |                      |
| 6   |              |                            |                  |                 |                  |                   |                      |
| 7   |              |                            |                  |                 |                  |                   |                      |
| 8   |              |                            |                  |                 |                  |                   |                      |

_Astuce : viser 8 à 15 tâches. Reprendre l'agenda + la boîte mail de la semaine écoulée._

## 2. Chronophages & irritants (synthèse)

- Tâche qui prend le plus de temps : **************\_\_**************
- Tâche la plus pénible / source d'erreurs : **************\_\_**************
- Tâche la plus répétitive : **************\_\_**************
- « Si je pouvais supprimer une corvée, ce serait… » : **************\_\_**************

## 3. Données & contraintes

- Outils imposés par l'entreprise : **************\_\_**************
- Données sensibles / confidentielles manipulées : **************\_\_**************
- Contraintes (réglementaires, validation hiérarchique, clients) : **************\_\_**************

## 4. Lecture rapide (matrice temps × valeur)

> Placer les tâches dans le quadrant.

|                       | **Faible valeur ajoutée**              | **Forte valeur ajoutée**           |
| --------------------- | -------------------------------------- | ---------------------------------- |
| **Beaucoup de temps** | 🎯 À automatiser/optimiser en priorité | ⚙️ À outiller pour aller plus vite |
| **Peu de temps**      | 🧹 À regrouper / supprimer             | ✅ À préserver                     |

**Top 5 candidats à l'optimisation** (issus du quadrant) :

1. **_ 2. _** 3. **_ 4. _** 5. \_\_\_

→ Reporter ces candidats dans **`04-grille-optimisations.md`**.
