...
async function main() {
  ...
  await fs.writeFile(
    OUTPUT,
    JSON.stringify({ count: worst.length, villes: worst }, null, 2),
    "utf-8",
  );
  ...
  for (let i = 0; i < worst.length; i += 10) {
    ...
    for (const w of chunk) {
      console.log(
        `  ${w.slug} | ${w.nameFr} (${w.departement} ${w.departementLabel}) | ${w.regionLabel} | pop ${w.population} | qs=${w.qualityScore} chars=${w.charsUnique}`,
      );
    }
  }
}
