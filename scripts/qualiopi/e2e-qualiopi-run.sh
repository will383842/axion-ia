#!/usr/bin/env bash
# Qualiopi — runner e2e SYSTÈME COMPLET (collectif + 1-to-1 AFEST), DB jetable pgvector.
#
# Provisionne une base Postgres+pgvector jetable, applique les migrations, lance
# LES DEUX chaînes e2e (formations collectives + coaching 1-to-1 AFEST) sur la
# même base, puis détruit le conteneur (trap EXIT). Verdict unique « tout le
# système Qualiopi vert bout-en-bout ».
#
...
echo "[e2e:qualiopi] 3/5 — migrations…"
pnpm exec prisma migrate deploy

echo "[e2e:qualiopi] 4/5 — e2e FORMATIONS COLLECTIVES (14 docs + attestations + factures + conformité 22 + BPF)…"
pnpm exec tsx scripts/qualiopi/e2e-formations-verif.ts

echo "[e2e:qualiopi] 5/5 — e2e 1-to-1 AFEST (kits financeurs + certificat + ventilation + BPF)…"
pnpm exec tsx scripts/qualiopi/e2e-afest-verif.ts

echo "[e2e:qualiopi] ✅ Système complet vert. Résultats : _AUDIT/VERIF-QUALIOPI-1TO1-AFEST-2026-06-14/{e2e-formations-results.json,e2e-results.json,pdf/}"
