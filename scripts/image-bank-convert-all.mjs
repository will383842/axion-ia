...
const MANIFEST_PATH = path.resolve(__dirname, '../../_AUDIT/image-bank-complet-2026/00-rename-manifest.json')
...
function findLogo(dir, sourceName) {
  ...
  if (sourceName.includes('...')) {
    ...
    try {
      ...
      const match = files.find(f => {
        ...
        return base.startsWith(beforeClean) && (!afterClean || base.endsWith(afterClean.replace(/\.png$/, '')))
      })
      if (match) return path.join(dir, match)
    } catch {}
  }
  ...
  try {
    ...
    const match = files.find(f => f.toLowerCase().replace(/\.png$/i, '').replace(/\.png$/i, '').includes(baseName.split('...')[0].trim()))
    if (match) return path.join(dir, match)
  } catch {}
...
async function convertImage(srcPath, slug, isLogo) {
  ...
  try {
    ...
    return { slug, status: 'OK', format: meta.format, w: meta.width, h: meta.height, webpKB: Math.round(webpSize/1024) }
  } catch (err) {
...
async function main() {
  ...
  const results = []
  let ok = 0, skip = 0, errors = 0, notFound = 0
  ...
  for (const entry of manifest) {
    ...
    if (!srcPath) {
      const result = { id: entry.id, slug: entry.targetSlug, status: `NOT_FOUND (${entry.sourceFolder}/${entry.source})` }
      results.push(result)
    ...
    if (result.status.startsWith('OK')) {
      console.log(`✅ [${entry.id}] ${entry.targetSlug.substring(0,55)} — ${result.webpKB}KB WebP`)
      ok++
  ...
  // Rapport JSON
  const reportPath = path.resolve(__dirname, '../../_AUDIT/image-bank-complet-2026/14-conversion-report.json')
