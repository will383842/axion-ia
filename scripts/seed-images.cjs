...
// ─── GEO-093 — ne jamais déclarer une miniature qui n'existe pas ─────────────
//
// Mesuré le 2026-08-16 : sur les 133 images seedées, **72 n'ont pas de fichier
// `-thumb.webp`**. Le seed écrivait pourtant `thumbnailPath` pour toutes → 72
// URLs de miniature en 404, servies aux visiteurs et aux crawlers.
//
// 🔑 Déclarer une URL morte est PIRE que ne rien déclarer : la colonne est
// nullable, et tous les consommateurs savent retomber sur `filePath` — qui,
// lui, existe pour les 133 (vérifié). Un `null` honnête affiche l'image ; un
// chemin qui ment affiche un trou.
//
// Les AVIF, elles, sont présentes pour les 133 : pas de garde nécessaire.
const fs = require("node:fs");
const path = require("node:path");

/** Racine des fichiers publics dans le conteneur (Next standalone). */
const RACINE_PUBLIC = process.env.SEED_PUBLIC_DIR || "/app/public";

/**
 * Rend le chemin relatif si le fichier existe, `null` sinon.
 *
 * Fail-soft : si la racine est introuvable (exécution hors conteneur), on
 * conserve le chemin plutôt que de tout passer à `null` — mieux vaut l'état
 * actuel qu'une régression massive due à un environnement inattendu.
 */
function cheminSiPresent(relatif) {
  try {
    if (!fs.existsSync(RACINE_PUBLIC)) return relatif;
    return fs.existsSync(path.join(RACINE_PUBLIC, relatif)) ? relatif : null;
  } catch {
    return relatif;
  }
}
    ...
    try {
      const asset = await prisma.imageAsset.upsert({
        ...
        create: {
          ...
          thumbnailPath: cheminSiPresent(`images/${entry.slug}-thumb.webp`),
      ...
      await prisma.imageAssetTranslation.upsert({
        ...
        // 🔴 GEO-089 — NE JAMAIS remettre ici `title`, `alt`, `caption` ni
        // `publishedAt`. Ils sont dans `create` uniquement, et c'est délibéré.
        //
        // Ce bloc les contenait. Comme le seed se déclenche sur le
        // `workflow_run` du déploiement, **chaque mise en production écrasait
        // les textes enrichis** — ceux qui décrivent réellement l'image — par la
        // dérivation mécanique du slug ci-dessus, identique pour toutes. C'est
        // pourquoi il ne faut pas relancer l'enrichissement avant ce correctif :
        // le travail serait reperdu au déploiement suivant.
        //
        // `publishedAt` avait un effet distinct : le remettre à `new Date()` à
        // chaque passage faisait bouger la date de publication de TOUTES les
        // images, donc les `lastmod` des sitemaps images — un signal de
        // fraîcheur mensonger envoyé aux moteurs.
        //
        // ⚠️ Ne les réintroduisez pas « pour que le seed soit vraiment
        // idempotent » : c'est exactement le raisonnement qui a produit le
        // défaut. Le seed garantit qu'une entrée EXISTE ; il n'est pas
        // propriétaire de son contenu éditorial.
        // Garde : `tests/unit/seo/seed-images-nepas-ecraser.spec.ts`.
        update: {
          slug: entry.slug,
          isPublished: true,
        },
