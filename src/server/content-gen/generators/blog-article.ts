 ...
 *
 * ARCHITECTURE 3-APPELS (2026-06-25) — voir audit content-gen profond.
 ...
 *   2) EXPANSION (boucle qualité max 2 itérations) : développe l'outline en
 *      bodyHtml complet par tranches de 4 sections (130-190 mots/section).
 */
...
// Plancher de longueur DUR : avec l'archi 3-appels la cible réaliste est 850+
...
- "metaTitle" : 50-60 caractères MAX, keyword principal au début.
- "metaDescription" : 140-155 caractères, phrase complète avec bénéfice clair, keyword naturel inclus.
- "directAnswer" : **40 à 80 mots** (ni moins, ni plus), réponse autonome et complète au sujet, citable seule par une IA (AI Overview / vocal).
...
- INTERDIT : section purement définitionnelle. L'angle est TOUJOURS ce qu'Axion-IA fait concrètement (angle opérationnel : cas d'usage réels, bénéfices mesurables, retour terrain). 0 prix en dur, 0 délai chiffré, 0 numéro de téléphone (contact@axion-ia.com uniquement).
...
export const blogArticleGenerator: Generator = {
  ...
  async generate(input: GeneratorBaseInput): Promise<GeneratorOutput> {
    ...
    // Mot-clé effectif pour les gates/SEO : primaryKeyword si présent, sinon le
    // topic synthétisé (les articles de campagne n'ont pas toujours de keyword).
    const effectiveKeyword = input.primaryKeyword ?? topic;
    ...
    // 1. KB retrieve — hybride FTS centré sur le sujet + secteur
    ...
    // P1-7 — Contexte glossaire IA injecté dans userPrompt.
    ...
    type BlogArticleParsed = Omit<PlanParsed, "outline"> & { bodyHtml: string };
...
{ title, metaTitle, metaDescription, slug, directAnswer, faq:[{q,a}×8], tags, keyTakeaway, expertTake, outline:[{h2,brief}×8] }`;
    ...
    for (let planAttempt = 0; planAttempt < 2 && !plan; planAttempt++) {
      const planResult = await routerGenerate({
        ...
        userPrompt:
          planAttempt === 0
            ? planUserPrompt
            : `${planUserPrompt}\n\n## Correction\nLe "title" et le "metaTitle" DOIVENT couvrir le sujet "${safeTopic}" (≤ 60 caractères ; « IA » accepté pour « intelligence artificielle »).`,
        maxTokens: 2600,
        temperature: 0.6,
      });
      ...
      // Gate title keyword (une réparation max — sinon on accepte tel quel, le
      // seoScore pénalisera). Matching robuste IA⇄intelligence artificielle.
    ...
    while (iteration < MAX_QUALITY_ITERATIONS) {
      ...
      // denses → corps long. Le helper concatène le HTML des tranches.
      ...
      const expandResult = await expandOutlineChunked({
        ...
        buildUserPrompt: ({ chunkText, isFirst, isLast }) =>
          `Rédige le CORPS HTML (sections de l'article) sur le sujet : "${safeTopic}" (intent ${safeIntent}, audience ${safeAudienceSize}).
...
{ bodyHtml }  (UNIQUEMENT les <h2> de CES sections, rien d'autre)`,
    ...
    if (!parsed) {
      throw new Error("blog-article: aucun corps valide après expansion");
    }

    // 3. Sanitize HTML final + section Sources déterministe + liens internes
    ...
    // DÉTERMINISTE (section « Sources ») avant le comptage, garantissant
    // citationCount ≥ liens sélectionnés (indépendant de Perplexity). Idempotent.
    parsed = {
      ...parsed,
    ...
    parsed = {
      ...parsed,
      bodyHtml: injectInternalLinks(parsed.bodyHtml, effectiveKeyword),
    };
