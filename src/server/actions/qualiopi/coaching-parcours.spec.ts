...
function baseInput(overrides: Record<string, unknown> = {}) {
  return {
    ...
    seances: [{ date: "2026-09-01T09:00:00Z" }, { date: "2026-09-08T09:00:00Z" }],
    ...overrides,
  };
}
...
beforeEach(() => {
  vi.clearAllMocks();
  mp.trainer.findUnique.mockResolvedValue({ id: TRAINER_ID, statut: "actif", nom: "Durand" });
...
describe("createCoachingParcoursAction", () => {
  ...
  it("REFUSE un formateur inactif — l'affectation exige un formateur actif", async () => {
    mp.trainer.findUnique.mockResolvedValue({ id: TRAINER_ID, statut: "archive", nom: "X" });
  ...
  it("REFUSE une séance dont la fin précède le début", async () => {
    const res = await createCoachingParcoursAction(
      baseInput({
        seances: [{ date: "2026-09-01T10:00:00Z", dateFin: "2026-09-01T09:00:00Z" }],
      }),
    );
    expect("error" in res).toBe(true);
    expect(mp.coachingSession.create).not.toHaveBeenCalled();
