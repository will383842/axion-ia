...
export async function signerPourStagiaireAction(input: {
  ...
  const session = await prisma.trainingSession.findUnique({
    ...
    select: {
      ...
      sessionFormateurs: {
        where: { trainerId: formateur.trainerId },
        select: { role: true },
      },
    },
  });
  ...
  const appartenance =
    session === null
      ? { estMembre: false }
      : resoudreAppartenance({
          estPrincipalFk: session.formateurPrincipalId === formateur.trainerId,
          roleSessionFormateur:
            (session.sessionFormateurs[0]?.role as RoleFormateur | undefined) ?? null,
        });
