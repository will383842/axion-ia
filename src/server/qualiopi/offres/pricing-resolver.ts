 ...
 */
export function resolveOffrePriceLabel(tierId: string, locale: "fr" | "en" = "fr"): string {
 ...
 */
export function verifyOffreCoherence(input: { tierId: string; tarifType: OffreTarifType }): {
  ok: boolean;
  ecarts: string[];
} {
  const ecarts: string[] = [];
