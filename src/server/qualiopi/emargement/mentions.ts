 ...
 */
export const MENTION_VERSION = "v1" as const;
...
export function mentionAttestation(p: ParametresMention): string {
  return `J'atteste avoir suivi ${p.demiJourneeLisible} du ${p.jourLisible} (${p.horaires}) de la formation « ${p.formationIntitule} », dispensée par ${p.organisme}.`;
}
