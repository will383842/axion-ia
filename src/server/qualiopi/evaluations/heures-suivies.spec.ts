 ...
 *     même stagiaire ;
 *   - « 0 h » se lisait sur un taux ENTIER : 20 minutes réelles sur 70 h
...
describe("heures suivies — une seule définition", () => {
  it("🔴 « 0 h » = minutes RÉELLES strictement nulles ; sans créneau, taux strictement nul", () => {
    const aucune = H["aucuneHeureSuivie"] as (p: Entree) => boolean;
    expect(typeof aucune).toBe("function");
    // 20 minutes réelles sur 70 h : le taux entier vaut 0 %, mais ce n'est PAS 0 h.
  ...
  it("🔴 minutes calculées sur les minutes réelles quand elles existent, jamais 0 si suivi", () => {
