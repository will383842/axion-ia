...
export function RecipientManager({ recipients }: { recipients: Recipient[] }): React.ReactElement {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end gap-2 rounded-lg border border-[#e7e0d6] bg-[#faf8f3] p-3">
        <label className="text-xs font-medium text-[#2a2520]">
          E-mail
          <input
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="mt-1 block w-56 rounded border border-[#e7e0d6] px-2 py-1 text-xs"
        ...
        <label className="text-xs font-medium text-[#2a2520]">
          ...
          <input
            ...
            className="mt-1 block w-40 rounded border border-[#e7e0d6] px-2 py-1 text-xs"
          />
        </label>
        <label className="text-xs font-medium text-[#2a2520]">
          Rôle
          <select
            ...
            className="mt-1 block rounded border border-[#e7e0d6] px-2 py-1 text-xs"
          >
      ...
      {error ? <p className="mb-3 text-xs text-[#b3261e]">{error}</p> : null}

      {recipients.length === 0 ? (
        <p className="text-sm text-[#6b635b]">Aucun destinataire. Ajoutez-en ci-dessus.</p>
      ) : (
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#e7e0d6] text-left text-[#6b635b]">
          ...
          <tbody>
            {recipients.map((r) => (
              <tr key={r.id} className="border-b border-[#f0ebe2]">
                ...
                <td>
                  <button
                    ...
                    className={`rounded px-1.5 py-0.5 ${r.actif ? "bg-[#eaf6ec] text-[#1e7d34]" : "bg-[#f3f0ea] text-[#6b635b]"}`}
                  >
                ...
                <td className="text-right">
                  <button
                    ...
                    disabled={pending}
                    onClick={() => remove(r.id)}
                    className="text-[#b3261e] hover:underline"
                  >
                    Supprimer
                  </button>
                </td>
