/**
 * AuditBenefits — « Ce qu'un audit IA vous apporte » : 5 bénéfices concrets en
 * rectangles verticaux sur une ligne + CTA ouvrant la popup du champ des
 * possibles IA.
 *
 * Refonte 2026-05-31 (Will) — header standard (H2 même taille que les autres
 * sections), très visuel. Le détail exhaustif par fonction vit dans la popup
 * (AuditCapabilitiesDialog). Couverture sémantique SEO/AEO côté serveur via la
 * sous-ligne des domaines.
 *
...
export function AuditBenefits({ isFr }: AuditBenefitsProps): ReactNode {
  return (
    <Section
      ...
      title={isFr ? "Ce qu'un audit IA vous apporte," : "What an AI audit gives you,"}
    ...
    >
      {/* 5 bénéfices — rectangles VERTICAUX (plus hauts que larges) sur une seule
          ligne dès md (768px). Breakpoint à md et non lg : à largeur CSS réduite
          (mise à l'échelle Windows) lg ne se déclenchait pas → fallback 2 cols
          larges. Accent en haut + padding resserré pour des cards étroites nettes. */}
      <ul className="grid list-none grid-cols-2 gap-3 p-0 sm:gap-4 md:grid-cols-5">
        {BENEFITS.map((b) => {
          const Icon = b.icon;
          return (
            <li
              key={b.titleFr}
              className="bg-paper border-border shadow-subtle hover:border-terracotta/50 hover:shadow-card flex h-full min-h-[16rem] flex-col overflow-hidden rounded-2xl border transition"
            >
