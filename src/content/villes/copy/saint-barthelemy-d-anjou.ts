// AUTO-GENERATED 2026-05-27 — T4 batch #25 Claude Code (Will).
// Saint-Barthélemy-d'Anjou (49, Maine-et-Loire) — est Angers, zones activités.

import type { VilleCopy } from "./types";

export const SAINT_BARTHELEMY_D_ANJOU_COPY: VilleCopy = {
  pitchFr:
    "Saint-Barthélemy-d'Anjou (49), commune dynamique à l'est d'Angers, concentre d'importantes zones d'activités économiques et près de 10 000 habitants. Axion-IA y intervient pour tous types d'activités : PME tertiaires, logistique, commerce, artisanat et services aux entreprises.",
  pitchEn:
    "Saint-Barthélemy-d'Anjou (49), commune dynamique à l'est d'Angers, concentre d'importantes zones d'activités économiques et près de 10 000 habitants. Axion-IA y intervient pour tous types d'activités : PME tertiaires, logistique, commerce, artisanat et services aux entreprises.",
  directAnswerFr:
    "Axion-IA est un cabinet de conseil en intelligence artificielle qui intervient à Saint-Barthélemy-d'Anjou (49). Nous accompagnons les TPE, PME et entreprises des zones d'activités bartholoméennes dans l'adoption de l'IA : audit Flash dès 490 € HT, interventions sur site, implémentation d'agents IA et coaching dirigeants 1-to-1. Tarifs publics.",
  directAnswerEn:
    "Axion-IA est un cabinet de conseil en intelligence artificielle qui intervient à Saint-Barthélemy-d'Anjou (49). Nous accompagnons les TPE, PME et entreprises des zones d'activités bartholoméennes dans l'adoption de l'IA : audit Flash dès 490 € HT, interventions sur site, implémentation d'agents IA et coaching dirigeants 1-to-1. Tarifs publics.",
  seoHook: "services aux & logistique transport",
  ecosystemFr:
    "Saint-Barthélemy-d'Anjou est un pôle économique majeur de l'est angevin avec ses zones d'activités (ZA de la Baumette, Parc d'activités Saint-Barthélemy). Tissu dense de PME tertiaires, logistique, commerce de gros, services aux entreprises et sièges sociaux régionaux. Proximité immédiate Angers Loire Métropole.",
  ecosystemEn:
    "Saint-Barthélemy-d'Anjou est un pôle économique majeur de l'est angevin avec ses zones d'activités (ZA de la Baumette, Parc d'activités Saint-Barthélemy). Tissu dense de PME tertiaires, logistique, commerce de gros, services aux entreprises et sièges sociaux régionaux. Proximité immédiate Angers Loire Métropole.",
  distancesFr:
    "Angers centre à 10 min. Gare TGV Angers Saint-Laud à 15 min. Aéroport Nantes Atlantique à 1h15 A11. Accès A11 direction Paris. Tramway via correspondance Angers est. Trélazé limitrophe.",
  distancesEn:
    "Angers centre à 10 min. Gare TGV Angers Saint-Laud à 15 min. Aéroport Nantes Atlantique à 1h15 A11. Accès A11 direction Paris. Tramway via correspondance Angers est. Trélazé limitrophe.",
  topSectorsNaf: [
    "Services aux entreprises & tertiaire",
    "Logistique & transport",
    "Commerce de gros & distribution",
    "PME industrielles & sièges régionaux",
    "Artisanat & BTP",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Saint-Barthélemy-d'Anjou : diagnostic adapté aux PME tertiaires, entreprises logistiques et commerces de gros des zones d'activités est-angevines. Audit Flash 490 € HT.",
      en: "Audit IA à Saint-Barthélemy-d'Anjou : diagnostic adapté aux PME tertiaires, entreprises logistiques et commerces de gros des zones d'activités est-angevines. Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Saint-Barthélemy-d'Anjou : formations pour équipes PME tertiaires, logistique et commerce de gros. Ateliers pratiques dans vos locaux de zone d'activités.",
      en: "Interventions IA sur site à Saint-Barthélemy-d'Anjou : formations pour équipes PME tertiaires, logistique et commerce de gros. Ateliers pratiques dans vos locaux de zone d'activités.",
    },
    implementation: {
      fr: "Implémentation IA à Saint-Barthélemy-d'Anjou : automatisation supply chain, agents conversationnels B2B, CRM augmenté, classification documentaire. Solutions adaptées PME et logistique.",
      en: "Implémentation IA à Saint-Barthélemy-d'Anjou : automatisation supply chain, agents conversationnels B2B, CRM augmenté, classification documentaire. Solutions adaptées PME et logistique.",
    },
    unAUn: {
      fr: "Coaching IA 1-to-1 à Saint-Barthélemy-d'Anjou : sessions pour dirigeants PME, responsables logistique et gérants commerces de gros souhaitant intégrer l'IA.",
      en: "Coaching IA 1-to-1 à Saint-Barthélemy-d'Anjou : sessions pour dirigeants PME, responsables logistique et gérants commerces de gros souhaitant intégrer l'IA.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Saint-Barthélemy-d'Anjou ?",
      a: "Oui, Axion-IA intervient à Saint-Barthélemy-d'Anjou et dans toutes les zones d'activités est-angevines (Trélazé, Verrières-en-Anjou). Nos consultants se déplacent sur site, démarrage sous 5 jours.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Saint-Barthélemy-d'Anjou ?",
      a: "L'Audit Flash IA à Saint-Barthélemy-d'Anjou démarre à 490 € HT pour une session de 4 heures. Diagnostic complet, ROI chiffré et plan d'action concret inclus. Tarifs publics.",
    },
    {
      q: "Accompagnez-vous les entreprises logistiques de Saint-Barthélemy ?",
      a: "Oui, nous accompagnons les entreprises logistiques à Saint-Barthélemy-d'Anjou : IA pour optimisation tournées, automatisation préparation commandes, traçabilité colis, prédiction stocks, agents conversationnels fournisseurs.",
    },
    {
      q: "Quels secteurs ciblez-vous à Saint-Barthélemy-d'Anjou ?",
      a: "Axion-IA accompagne tous types d'activités à Saint-Barthélemy-d'Anjou : PME tertiaires, logistique, commerce de gros, distribution, artisanat BTP et services aux entreprises. Pas de restriction sectorielle.",
    },
    {
      q: "Comment se déroule une mission IA dans les zones d'activités ?",
      a: "Nos consultants seniors se déplacent directement dans vos locaux de zone d'activités à Saint-Barthélemy-d'Anjou. Diagnostic, ateliers pratiques, livraison outils IA configurés. Frais déplacement Angers inclus (10 min).",
    },
  ],
};
