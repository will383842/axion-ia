// AUTO-GENERATED 2026-05-27 — T4 batch #20 Claude Code (Will).
// Louveciennes (78, Yvelines) — résidentielle prestige, aqueduc de Louveciennes.

import type { VilleCopy } from "./types";

export const LOUVECIENNES_COPY: VilleCopy = {
  pitchFr:
    "Louveciennes (78), commune résidentielle de prestige des Yvelines, entre Bougival et Marly-le-Roi, célèbre pour son aqueduc historique et son patrimoine impressionniste (Pissarro, Sisley). Tissu de TPE artisans, professions libérales haut de gamme, PME tertiaires, consultants et commerces de proximité. Axion-IA y intervient pour tous types d'activités de l'ouest parisien.",
  pitchEn:
    "Louveciennes (78), commune résidentielle de prestige des Yvelines, entre Bougival et Marly-le-Roi, célèbre pour son aqueduc historique et son patrimoine impressionniste (Pissarro, Sisley). Tissu de TPE artisans, professions libérales haut de gamme, PME tertiaires, consultants et commerces de proximité. Axion-IA y intervient pour tous types d'activités de l'ouest parisien.",
  directAnswerFr:
    "Axion-IA est un cabinet d'experts IA seniors qui intervient à Louveciennes (78). Nous accompagnons TPE et PME : professions libérales, consultants, PME tertiaires, artisans et commerçants de cette commune résidentielle de prestige de l'ouest parisien, entre Bougival et Marly-le-Roi. Audit sur place sur mesure.",
  directAnswerEn:
    "Axion-IA est un cabinet d'experts IA seniors qui intervient à Louveciennes (78). Nous accompagnons TPE et PME : professions libérales, consultants, PME tertiaires, artisans et commerçants de cette commune résidentielle de prestige de l'ouest parisien, entre Bougival et Marly-le-Roi. Audit sur place sur mesure.",
  seoHook: "professions libérales & conseil services",
  ecosystemFr:
    "Louveciennes est une commune résidentielle haut de gamme de l'ouest des Yvelines, entre Bougival et Marly-le-Roi. Aqueduc historique de Louveciennes et patrimoine impressionniste (Pissarro, Sisley). Tissu économique : professions libérales (juridique, santé, patrimoine), PME tertiaires et cabinets conseil, TPE artisans, commerces de village, consultants indépendants. Communes voisines : Bougival, Marly-le-Roi, Le Pecq, La Celle-Saint-Cloud, Vaucresson.",
  ecosystemEn:
    "Louveciennes est une commune résidentielle haut de gamme de l'ouest des Yvelines, entre Bougival et Marly-le-Roi. Aqueduc historique de Louveciennes et patrimoine impressionniste (Pissarro, Sisley). Tissu économique : professions libérales (juridique, santé, patrimoine), PME tertiaires et cabinets conseil, TPE artisans, commerces de village, consultants indépendants. Communes voisines : Bougival, Marly-le-Roi, Le Pecq, La Celle-Saint-Cloud, Vaucresson.",
  distancesFr:
    "Gare de Louveciennes (Transilien L, 25 min Paris Saint-Lazare). A13/A86 à proximité. La Défense à 15 min, Versailles à 15 min, Saint-Germain-en-Laye à 10 min. Aéroport Paris-Orly à 35 min, Roissy-CDG à 50 min.",
  distancesEn:
    "Gare de Louveciennes (Transilien L, 25 min Paris Saint-Lazare). A13/A86 à proximité. La Défense à 15 min, Versailles à 15 min, Saint-Germain-en-Laye à 10 min. Aéroport Paris-Orly à 35 min, Roissy-CDG à 50 min.",
  topSectorsNaf: [
    "Professions libérales haut de gamme (juridique, santé, patrimoine)",
    "Conseil & services aux entreprises",
    "PME tertiaires & cabinets conseil",
    "Commerce de village & artisanat",
    "Immobilier & gestion de patrimoine",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Louveciennes : diagnostic adapté aux professions libérales, PME tertiaires, consultants et TPE artisans de l'ouest parisien. ROI chiffré, Audit sur place {{price:audit-flash|flat}}.",
      en: "Audit IA à Louveciennes : diagnostic adapté aux professions libérales, PME tertiaires, consultants et TPE artisans de l'ouest parisien. ROI chiffré, Audit sur place {{price:audit-flash|flat}}.",
    },
    interventions: {
      fr: "Interventions IA sur site à Louveciennes : formats adaptés au cabinet, au bureau ou en visio. Vos équipes repartent autonomes sur les outils IA configurés pour leur métier.",
      en: "Interventions IA sur site à Louveciennes : formats adaptés au cabinet, au bureau ou en visio. Vos équipes repartent autonomes sur les outils IA configurés pour leur métier.",
    },
    implementation: {
      fr: "Implémentation IA à Louveciennes : automatisation gestion clients, agents conversationnels, classification dossiers, CRM augmenté cabinets, gestion patrimoine assistée IA.",
      en: "Implémentation IA à Louveciennes : automatisation gestion clients, agents conversationnels, classification dossiers, CRM augmenté cabinets, gestion patrimoine assistée IA.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Louveciennes : sessions dans votre cabinet, bureau ou en visio. Professions libérales, dirigeants PME, consultants et gérants de l'ouest parisien souhaitant maîtriser l'IA.",
      en: "Coaching 1-to-1 à Louveciennes : sessions dans votre cabinet, bureau ou en visio. Professions libérales, dirigeants PME, consultants et gérants de l'ouest parisien souhaitant maîtriser l'IA.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Louveciennes dans les Yvelines ?",
      a: "Oui, Axion-IA intervient à Louveciennes et dans tout l'ouest des Yvelines (Bougival, Marly-le-Roi, Le Pecq, La Celle-Saint-Cloud, Vaucresson). Nos consultants se déplacent sur site ou en visio.",
    },
    {
      q: "Quel est le prix d'un audit IA à Louveciennes ?",
      a: "L'Audit sur place IA à Louveciennes démarre à {{price:audit-flash|flat}} pour 4 heures de diagnostic. Il couvre l'analyse de vos processus métier, l'identification des cas d'usage IA prioritaires et un plan d'action chiffré avec ROI estimé.",
    },
    {
      q: "Accompagnez-vous les professions libérales à Louveciennes ?",
      a: "Oui, nous accompagnons les professions libérales de Louveciennes (avocats, médecins, conseillers patrimoine, architectes) sur leurs cas IA : automatisation administrative, classification dossiers, rédaction assistée, gestion clients.",
    },
    {
      q: "Proposez-vous des formations IA pour les PME à Louveciennes ?",
      a: "Oui, nos interventions IA sur site à Louveciennes sont adaptées aux PME tertiaires et cabinets conseil. Formats demi-journée ou journée, vos équipes repartent autonomes sur les outils IA configurés métier.",
    },
    {
      q: "Comment se déroule une mission IA à Louveciennes ?",
      a: "Nos consultants rejoignent Louveciennes via le Transilien L (25 min Paris Saint-Lazare) ou par l'A13/A86. La Défense à 15 min. Frais de déplacement Île-de-France inclus ou facturés selon forfait.",
    },
  ],
};
