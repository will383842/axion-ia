// AUTO-GENERATED 2026-05-27 — T4 batch #21 Claude Code (Will).
// Saint-Florent-sur-Cher (18, Cher) — 6 463 hab, vallée du Cher, industrie, agglo Bourges sud.

import type { VilleCopy } from "./types";

export const SAINT_FLORENT_SUR_CHER_COPY: VilleCopy = {
  pitchFr:
    "Saint-Florent-sur-Cher (18), commune de la vallée du Cher au sud de Bourges, possède un tissu industriel historique et tertiaire. TPE artisans, PME industrielles, commerces, agriculture céréalière, services. Axion-IA y conduit ses missions auprès des entreprises locales.",
  pitchEn:
    "Saint-Florent-sur-Cher (18), commune de la vallée du Cher au sud de Bourges, possède un tissu industriel historique et tertiaire. TPE artisans, PME industrielles, commerces, agriculture céréalière, services. Axion-IA y conduit ses missions auprès des entreprises locales.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes seniors en intelligence artificielle qui intervient à Saint-Florent-sur-Cher (18). Nous accompagnons TPE artisans, PME industrielles, commerçants, exploitants agricoles et toutes les activités de la vallée du Cher au sud de Bourges. Audit sur place sur mesure.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes seniors en intelligence artificielle qui intervient à Saint-Florent-sur-Cher (18). Nous accompagnons TPE artisans, PME industrielles, commerçants, exploitants agricoles et toutes les activités de la vallée du Cher au sud de Bourges. Audit sur place sur mesure.",
  seoHook: "industrie métallurgie & tertiaires services",
  ecosystemFr:
    "Saint-Florent-sur-Cher est une commune industrielle et résidentielle de la vallée du Cher, au sud de Bourges. Tissu de TPE artisans, PME industrielles (métallurgie, mécanique), commerces de proximité, agriculture céréalière, services aux entreprises. Intégrée à Bourges Plus.",
  ecosystemEn:
    "Saint-Florent-sur-Cher est une commune industrielle et résidentielle de la vallée du Cher, au sud de Bourges. Tissu de TPE artisans, PME industrielles (métallurgie, mécanique), commerces de proximité, agriculture céréalière, services aux entreprises. Intégrée à Bourges Plus.",
  distancesFr:
    "Bourges centre à 15 min. Gare SNCF Bourges à 20 min. Saint-Amand-Montrond à 35 min. Paris à 2h20 via A71. Châteauroux à 50 min.",
  distancesEn:
    "Bourges centre à 15 min. Gare SNCF Bourges à 20 min. Saint-Amand-Montrond à 35 min. Paris à 2h20 via A71. Châteauroux à 50 min.",
  topSectorsNaf: [
    "Industrie métallurgie & mécanique",
    "TPE artisanat BTP",
    "PME tertiaires & services",
    "Agriculture céréalière",
    "Commerces de proximité",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Saint-Florent-sur-Cher : diagnostic adapté aux TPE artisans, PME industrielles (métallurgie, mécanique), commerçants et exploitants agricoles de la vallée du Cher. Audit sur place {{price:audit-flash|flat}} avec ROI quantifié.",
      en: "Audit IA à Saint-Florent-sur-Cher : diagnostic adapté aux TPE artisans, PME industrielles (métallurgie, mécanique), commerçants et exploitants agricoles de la vallée du Cher. Audit sur place {{price:audit-flash|flat}} avec ROI quantifié.",
    },
    interventions: {
      fr: "Interventions IA sur site à Saint-Florent-sur-Cher : formats adaptés aux industriels, artisans, commerçants, exploitants agricoles. Vos équipes repartent autonomes sur outils IA.",
      en: "Interventions IA sur site à Saint-Florent-sur-Cher : formats adaptés aux industriels, artisans, commerçants, exploitants agricoles. Vos équipes repartent autonomes sur outils IA.",
    },
    implementation: {
      fr: "Implémentation IA à Saint-Florent-sur-Cher : automatisation production industrielle, maintenance prédictive, gestion qualité, agents conversationnels commerce, optimisation logistique agricole.",
      en: "Implémentation IA à Saint-Florent-sur-Cher : automatisation production industrielle, maintenance prédictive, gestion qualité, agents conversationnels commerce, optimisation logistique agricole.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Saint-Florent-sur-Cher : sessions dans vos locaux industriels, commerciaux ou en visio. Dirigeants TPE/PME, gérants industrie, commerçants, exploitants agricoles vallée du Cher.",
      en: "Coaching 1-to-1 à Saint-Florent-sur-Cher : sessions dans vos locaux industriels, commerciaux ou en visio. Dirigeants TPE/PME, gérants industrie, commerçants, exploitants agricoles vallée du Cher.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Saint-Florent-sur-Cher et vallée du Cher ?",
      a: "Oui, Axion-IA intervient à Saint-Florent-sur-Cher et dans toute la vallée du Cher (Bourges, Mehun-sur-Yèvre, Lunery, Chârost). Consultants depuis Bourges (15 min). Couverture complète agglo Bourges Plus sud.",
    },
    {
      q: "Travaillez-vous avec les PME industrielles du Cher ?",
      a: "Oui, nous accompagnons les PME industrielles (métallurgie, mécanique) de Saint-Florent-sur-Cher sur leurs cas IA : automatisation production, maintenance prédictive, contrôle qualité, gestion stocks, optimisation supply chain.",
    },
    {
      q: "Quels services pour les TPE artisans locaux ?",
      a: "Pour les TPE artisans à Saint-Florent-sur-Cher : audit sur place {{price:audit-flash|flat}}, intervention 1 journée formation, coaching 1-to-1. Cas typiques : automatisation devis, planning chantier, gestion stocks, facturation.",
    },
    {
      q: "Accompagnez-vous les exploitants agricoles ?",
      a: "Oui, nous accompagnons les exploitants agricoles céréaliers de la vallée du Cher : optimisation logistique récoltes, gestion parcellaire IA, prévision rendements, automatisation suivi cultures, traçabilité production.",
    },
    {
      q: "Comment se déroule une mission à Saint-Florent-sur-Cher ?",
      a: "Nos consultants viennent de Bourges (15 min) ou de Paris via A71 (2h20). Intervention sur site dans vos locaux industriels ou en visio. Multi-jours possibles.",
    },
  ],
};
