// AUTO-GENERATED 2026-05-27 — T4 batch #21 Claude Code (Will).
// Saint-Avé (56, Morbihan) — agglo Vannes, commune résidentielle dynamique.

import type { VilleCopy } from "./types";

export const SAINT_AVE_COPY: VilleCopy = {
  pitchFr:
    "Saint-Avé (56), commune résidentielle dynamique de l'agglomération de Vannes (12 470 hab), conjugue cadre de vie et tissu économique tertiaire en plein développement. TPE et PME services, santé (centre hospitalier spécialisé), commerces de proximité, artisanat, BTP, écoles, professions libérales. Axion-IA accompagne tout le tissu professionnel local.",
  pitchEn:
    "Saint-Avé (56), commune résidentielle dynamique de l'agglomération de Vannes (12 470 hab), conjugue cadre de vie et tissu économique tertiaire en plein développement. TPE et PME services, santé (centre hospitalier spécialisé), commerces de proximité, artisanat, BTP, écoles, professions libérales. Axion-IA accompagne tout le tissu professionnel local.",
  directAnswerFr:
    "Axion-IA est un cabinet d'experts IA seniors qui intervient à Saint-Avé (56). Nous accompagnons TPE et PME : services et tertiaire de l'agglomération de Vannes, professionnels de santé du centre hospitalier, commerces de proximité, artisans, BTP, professions libérales, écoles. Audit sur place sur mesure.",
  directAnswerEn:
    "Axion-IA est un cabinet d'experts IA seniors qui intervient à Saint-Avé (56). Nous accompagnons TPE et PME : services et tertiaire de l'agglomération de Vannes, professionnels de santé du centre hospitalier, commerces de proximité, artisans, BTP, professions libérales, écoles. Audit sur place sur mesure.",
  seoHook: "services tertiaires & santé secteur",
  ecosystemFr:
    "Saint-Avé est une commune morbihannaise de la première couronne de Vannes, à la croissance résidentielle soutenue. Tissu TPE/PME tertiaire diversifié : services, santé (centre hospitalier spécialisé), commerce de proximité, artisanat, BTP, professions libérales, secteur éducatif.",
  ecosystemEn:
    "Saint-Avé est une commune morbihannaise de la première couronne de Vannes, à la croissance résidentielle soutenue. Tissu TPE/PME tertiaire diversifié : services, santé (centre hospitalier spécialisé), commerce de proximité, artisanat, BTP, professions libérales, secteur éducatif.",
  distancesFr:
    "Vannes centre à 10 min. Gare SNCF de Vannes (TGV Paris-Quimper) à 15 min. Paris-Montparnasse à 2 h 30 en TGV direct. Aéroport Lorient Bretagne Sud à 50 min. Nantes à 1 h 30. Voie express RN 165 à 5 min.",
  distancesEn:
    "Vannes centre à 10 min. Gare SNCF de Vannes (TGV Paris-Quimper) à 15 min. Paris-Montparnasse à 2 h 30 en TGV direct. Aéroport Lorient Bretagne Sud à 50 min. Nantes à 1 h 30. Voie express RN 165 à 5 min.",
  topSectorsNaf: [
    "Services tertiaires et professions libérales",
    "Santé et secteur hospitalier",
    "Commerce de proximité",
    "Artisanat et BTP",
    "Éducation et services aux particuliers",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Saint-Avé : diagnostic adapté aux TPE artisans, PME services de l'agglomération de Vannes, professionnels de santé, commerces de proximité, BTP, professions libérales. ROI chiffré, Audit sur place {{price:audit-flash|flat}}.",
      en: "Audit IA à Saint-Avé : diagnostic adapté aux TPE artisans, PME services de l'agglomération de Vannes, professionnels de santé, commerces de proximité, BTP, professions libérales. ROI chiffré, Audit sur place {{price:audit-flash|flat}}.",
    },
    interventions: {
      fr: "Interventions IA sur site à Saint-Avé : formats adaptés cabinet libéral, structure de santé, boutique de proximité, bureau tertiaire. Équipes formées à l'autonomie sur les outils IA.",
      en: "Interventions IA sur site à Saint-Avé : formats adaptés cabinet libéral, structure de santé, boutique de proximité, bureau tertiaire. Équipes formées à l'autonomie sur les outils IA.",
    },
    implementation: {
      fr: "Implémentation IA à Saint-Avé : gestion rendez-vous santé, dossier patient, CRM commerces de proximité, devis et planning BTP, automatisation administrative professions libérales.",
      en: "Implémentation IA à Saint-Avé : gestion rendez-vous santé, dossier patient, CRM commerces de proximité, devis et planning BTP, automatisation administrative professions libérales.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Saint-Avé : sessions dans vos locaux agglo Vannes, cabinet libéral, structure de santé ou en visio. Dirigeants TPE/PME services, professionnels de santé, commerçants, artisans, libéraux.",
      en: "Coaching 1-to-1 à Saint-Avé : sessions dans vos locaux agglo Vannes, cabinet libéral, structure de santé ou en visio. Dirigeants TPE/PME services, professionnels de santé, commerçants, artisans, libéraux.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Saint-Avé et sur l'agglomération de Vannes ?",
      a: "Oui, Axion-IA intervient à Saint-Avé et dans toute l'agglomération de Vannes. Vannes centre à 10 min, gare TGV à 15 min, voie express RN 165 à 5 min.",
    },
    {
      q: "Tarifs audit IA à Saint-Avé  : quel budget prévoir ?",
      a: "L'Audit sur place démarre à {{price:audit-flash|flat}}. Tarifs publics consultables en ligne, adaptés aux TPE artisans et PME services de l'agglomération de Vannes.",
    },
    {
      q: "Travaillez-vous avec les professionnels de santé du centre hospitalier ?",
      a: "Oui, gestion rendez-vous, dossier patient, télémédecine, automatisation administrative, RGPD santé, reporting qualité pour structures de santé et libéraux de Saint-Avé.",
    },
    {
      q: "Accompagnez-vous les professions libérales et commerces de proximité ?",
      a: "Oui, gestion clientèle, facturation, agenda, marketing local, fidélisation, automatisation administrative pour libéraux et commerces de proximité de Saint-Avé.",
    },
    {
      q: "Comment accéder à Saint-Avé depuis Paris ou Nantes ?",
      a: "Gare TGV de Vannes à 15 min (Paris-Montparnasse à 2 h 30 en TGV direct). Nantes à 1 h 30 via RN 165. Aéroport Lorient à 50 min. Frais de déplacement facturés à part.",
    },
  ],
};
