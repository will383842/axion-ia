// AUTO-GENERATED 2026-05-27 — T4 batch #24 Claude Code (Will).
// Bellegarde (30, Gard) — Costières, entre Nîmes et Arles.

import type { VilleCopy } from "./types";

export const BELLEGARDE_COPY: VilleCopy = {
  pitchFr:
    "Bellegarde (30), commune gardoise des Costières entre Nîmes et Arles, carrefour agricole et logistique. Tissu TPE/PME viticulture Costières de Nîmes, agriculture (fruits, légumes), logistique, commerces et artisanat BTP. Axion-IA couvre l'ensemble des activités professionnelles présentes des Costières.",
  pitchEn:
    "Bellegarde (30), commune gardoise des Costières entre Nîmes et Arles, carrefour agricole et logistique. Tissu TPE/PME viticulture Costières de Nîmes, agriculture (fruits, légumes), logistique, commerces et artisanat BTP. Axion-IA couvre l'ensemble des activités professionnelles présentes des Costières.",
  directAnswerFr:
    "Axion-IA est un cabinet de conseil en intelligence artificielle qui intervient à Bellegarde (30). Nous accompagnons les TPE, PME et ETI locales : viticulteurs Costières de Nîmes, exploitants agricoles, entreprises logistique, commerçants et artisans BTP entre Nîmes et Arles. Audit Flash à partir de 490 € HT.",
  directAnswerEn:
    "Axion-IA est un cabinet de conseil en intelligence artificielle qui intervient à Bellegarde (30). Nous accompagnons les TPE, PME et ETI locales : viticulteurs Costières de Nîmes, exploitants agricoles, entreprises logistique, commerçants et artisans BTP entre Nîmes et Arles. Audit Flash à partir de 490 € HT.",
  seoHook: "viticulture costières & agriculture",
  ecosystemFr:
    "Bellegarde (~8 000 hab) est une commune des Costières entre Nîmes (20 min) et Arles (25 min). Viticulture Costières de Nîmes AOP, agriculture (arboriculture, maraîchage), logistique axe Nîmes-Arles, commerces, artisanat BTP et services aux entreprises. Carrefour agricole gardois.",
  ecosystemEn:
    "Bellegarde (~8 000 hab) est une commune des Costières entre Nîmes (20 min) et Arles (25 min). Viticulture Costières de Nîmes AOP, agriculture (arboriculture, maraîchage), logistique axe Nîmes-Arles, commerces, artisanat BTP et services aux entreprises. Carrefour agricole gardois.",
  distancesFr:
    "Nîmes à 20 min (N113). Arles à 25 min. Beaucaire à 10 min. Aéroport Nîmes-Garons à 15 min. Gare TGV Nîmes à 25 min. Accès A54 Nîmes-Arles.",
  distancesEn:
    "Nîmes à 20 min (N113). Arles à 25 min. Beaucaire à 10 min. Aéroport Nîmes-Garons à 15 min. Gare TGV Nîmes à 25 min. Accès A54 Nîmes-Arles.",
  topSectorsNaf: [
    "Viticulture Costières de Nîmes",
    "Agriculture (arboriculture, maraîchage)",
    "Logistique axe Nîmes-Arles",
    "Commerce de proximité",
    "Artisanat BTP",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Bellegarde : diagnostic adapté aux TPE/PME viticoles Costières, exploitants agricoles, logisticiens et commerçants entre Nîmes et Arles. Audit Flash 490 € HT.",
      en: "Audit IA à Bellegarde : diagnostic adapté aux TPE/PME viticoles Costières, exploitants agricoles, logisticiens et commerçants entre Nîmes et Arles. Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Bellegarde : formats adaptés aux caves viticoles, exploitations agricoles, entreprises logistique et commerces des Costières.",
      en: "Interventions IA sur site à Bellegarde : formats adaptés aux caves viticoles, exploitations agricoles, entreprises logistique et commerces des Costières.",
    },
    implementation: {
      fr: "Implémentation IA à Bellegarde : prédiction vendanges Costières, tri qualité fruits, optimisation supply chain Nîmes-Arles, agents conversationnels commerce.",
      en: "Implémentation IA à Bellegarde : prédiction vendanges Costières, tri qualité fruits, optimisation supply chain Nîmes-Arles, agents conversationnels commerce.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Bellegarde : sessions dirigeants TPE/PME viticoles, agriculteurs, logisticiens et commerçants souhaitant intégrer l'IA dans les Costières.",
      en: "Coaching 1-to-1 à Bellegarde : sessions dirigeants TPE/PME viticoles, agriculteurs, logisticiens et commerçants souhaitant intégrer l'IA dans les Costières.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Bellegarde et dans les Costières ?",
      a: "Oui, Axion-IA intervient à Bellegarde et dans toutes les Costières (Beaucaire, Saint-Gilles, Jonquières-Saint-Vincent, Manduel). Nos consultants viennent de Nîmes (20 min) ou Arles (25 min).",
    },
    {
      q: "Accompagnez-vous les viticulteurs Costières de Nîmes ?",
      a: "Oui, nous accompagnons les viticulteurs Costières de Nîmes : prédiction vendanges, classification parcelles, automatisation traçabilité, optimisation vente directe et export.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Bellegarde ?",
      a: "L'Audit Flash commence à 490 € HT. Il couvre le diagnostic complet de vos processus métier, l'identification des cas d'usage IA prioritaires et un plan d'action chiffré avec ROI estimé.",
    },
    {
      q: "Travaillez-vous avec les exploitants agricoles ?",
      a: "Oui, nous accompagnons les exploitants agricoles des Costières : arboriculteurs, maraîchers. Tri qualité par vision IA, prédiction rendements, automatisation traçabilité, vente directe.",
    },
    {
      q: "Quels délais pour démarrer une mission IA à Bellegarde ?",
      a: "Démarrage sous 5 jours ouvrés après validation. Nos consultants se déplacent sur site à Bellegarde. Possibilité de sessions visio pour les phases de cadrage et suivi.",
    },
  ],
};
