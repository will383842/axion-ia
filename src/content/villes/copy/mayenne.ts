// AUTO-GENERATED 2026-05-27 — T4 batch #25 Claude Code (Will).
// Mayenne (53, Mayenne) — Sous-préfecture, château carolingien, bords Mayenne.

import type { VilleCopy } from "./types";

export const MAYENNE_COPY: VilleCopy = {
  pitchFr:
    "Mayenne (53), sous-préfecture du nord de la Mayenne aux bords de la rivière éponyme, dotée d'un château carolingien classé, concentre un tissu de TPE artisans, PME agroalimentaires et industrielles, commerces de centre-ville, professions libérales et prestataires de services. Axion-IA y intervient pour tous types d'activités.",
  pitchEn:
    "Mayenne (53), sous-préfecture du nord de la Mayenne aux bords de la rivière éponyme, dotée d'un château carolingien classé, concentre un tissu de TPE artisans, PME agroalimentaires et industrielles, commerces de centre-ville, professions libérales et prestataires de services. Axion-IA y intervient pour tous types d'activités.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Mayenne (53). Nous accompagnons tous types d'activités : TPE artisans, PME agroalimentaires et industrielles, commerçants, professions libérales, éleveurs, services aux entreprises et acteurs du tourisme patrimonial du nord-Mayenne.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Mayenne (53). Nous accompagnons tous types d'activités : TPE artisans, PME agroalimentaires et industrielles, commerçants, professions libérales, éleveurs, services aux entreprises et acteurs du tourisme patrimonial du nord-Mayenne.",
  ecosystemFr:
    "Mayenne est la sous-préfecture du nord du département, aux bords de la rivière Mayenne. Château carolingien (peintures murales les plus anciennes d'Europe), patrimoine remarquable. Économie : agroalimentaire (lait, viande), industrie (plasturgie, mécanique), artisanat, commerces, services, élevage bovin laitier.",
  ecosystemEn:
    "Mayenne est la sous-préfecture du nord du département, aux bords de la rivière Mayenne. Château carolingien (peintures murales les plus anciennes d'Europe), patrimoine remarquable. Économie : agroalimentaire (lait, viande), industrie (plasturgie, mécanique), artisanat, commerces, services, élevage bovin laitier.",
  distancesFr:
    "Gare de Mayenne (TER vers Laval 30 min). Gare TGV Laval à 30 km. Aéroport Rennes-Bretagne à 120 km. Accès N162 (Laval-Mayenne-Fougères).",
  distancesEn:
    "Gare de Mayenne (TER vers Laval 30 min). Gare TGV Laval à 30 km. Aéroport Rennes-Bretagne à 120 km. Accès N162 (Laval-Mayenne-Fougères).",
  topSectorsNaf: [
    "Agroalimentaire (lait, viande)",
    "Industrie (plasturgie, mécanique)",
    "Élevage bovin laitier",
    "Artisanat BTP & services",
    "Commerce de centre-ville",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Mayenne : diagnostic adapté aux TPE artisans, PME agroalimentaires, industriels et commerçants du nord-Mayenne. ROI chiffré, Audit Flash 490 € HT.",
      en: "Audit IA à Mayenne : diagnostic adapté aux TPE artisans, PME agroalimentaires, industriels et commerçants du nord-Mayenne. ROI chiffré, Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Mayenne : formations adaptées aux PME agroalimentaires, industriels plasturgie/mécanique, artisans, éleveurs et commerçants. Vos équipes repartent autonomes.",
      en: "Interventions IA sur site à Mayenne : formations adaptées aux PME agroalimentaires, industriels plasturgie/mécanique, artisans, éleveurs et commerçants. Vos équipes repartent autonomes.",
    },
    implementation: {
      fr: "Implémentation IA à Mayenne : contrôle qualité agroalimentaire, maintenance prédictive industrie, traçabilité élevage laitier, automatisation processus plasturgie et logistique.",
      en: "Implémentation IA à Mayenne : contrôle qualité agroalimentaire, maintenance prédictive industrie, traçabilité élevage laitier, automatisation processus plasturgie et logistique.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Mayenne : sessions dans vos locaux industriels, exploitation, commerce ou en visio. Dirigeants TPE/PME agroalimentaires, industriels, artisans, commerçants.",
      en: "Coaching 1-to-1 à Mayenne : sessions dans vos locaux industriels, exploitation, commerce ou en visio. Dirigeants TPE/PME agroalimentaires, industriels, artisans, commerçants.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Mayenne et dans le nord du département ?",
      a: "Oui, Axion-IA intervient à Mayenne et dans tout le nord-Mayenne (Gorron, Ambrières-les-Vallées, Lassay-les-Châteaux, Villaines-la-Juhel). Nos consultants se déplacent depuis Laval (30 min N162).",
    },
    {
      q: "Travaillez-vous avec les PME agroalimentaires et laitières du nord-Mayenne ?",
      a: "Oui, nous accompagnons les PME agroalimentaires et laitières du bassin de Mayenne sur leurs cas IA : contrôle qualité, traçabilité laitière, prédiction production, automatisation logistique froide et gestion stocks.",
    },
    {
      q: "Quel est le tarif d'un audit IA pour une TPE à Mayenne ?",
      a: "L'Audit Flash démarre à 490 € HT pour 4 heures de diagnostic complet. Adapté aux TPE artisans, PME agroalimentaires et industriels du nord-Mayenne. Tarifs publics, sans surprise.",
    },
    {
      q: "Proposez-vous des formations IA pour l'industrie plasturgique ?",
      a: "Oui, nos interventions forment vos équipes aux outils IA adaptés à la plasturgie : contrôle qualité par vision, maintenance prédictive, optimisation cycles de production, automatisation tri et conditionnement.",
    },
    {
      q: "Comment se passe une mission IA à Mayenne depuis Laval ?",
      a: "Nos consultants viennent de Laval (30 min N162). Gare TER Mayenne desservie. Pour les missions multi-jours, hébergement local possible. Format mixte sur site + visio. Tarifs publics.",
    },
  ],
};
