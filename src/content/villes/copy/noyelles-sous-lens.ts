// AUTO-GENERATED 2026-05-27 — T4 batch #23 Claude Code (Will).
// Noyelles-sous-Lens (62, Pas-de-Calais) — Bassin minier, 6 854 hab.

import type { VilleCopy } from "./types";

export const NOYELLES_SOUS_LENS_COPY: VilleCopy = {
  pitchFr:
    "Noyelles-sous-Lens (62), commune du bassin minier du Pas-de-Calais, reunit pres de 6 850 habitants. Situee dans l'agglomeration lensoise, son economie repose sur les TPE de services, les PME commerciales, l'artisanat et les activites tertiaires. Axion-IA y intervient pour tous types d'activites.",
  pitchEn:
    "Noyelles-sous-Lens (62), commune du bassin minier du Pas-de-Calais, reunit pres de 6 850 habitants. Situee dans l'agglomeration lensoise, son economie repose sur les TPE de services, les PME commerciales, l'artisanat et les activites tertiaires. Axion-IA y intervient pour tous types d'activites.",
  seoHook: "bassin minier, commerce & tertiaire",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient a Noyelles-sous-Lens (62). Nous accompagnons les TPE, PME et ETI du bassin minier lensois : commercants, artisans, prestataires de services et professions liberales. Audit Flash a partir de 490 EUR HT.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient a Noyelles-sous-Lens (62). Nous accompagnons les TPE, PME et ETI du bassin minier lensois : commercants, artisans, prestataires de services et professions liberales. Audit Flash a partir de 490 EUR HT.",
  ecosystemFr:
    "Noyelles-sous-Lens est situee dans l'agglomeration de Lens (3 km), au coeur du bassin minier. Tissu economique compose de TPE de services, PME commerciales, artisans BTP, professions liberales et activites tertiaires liees a la proximite du Louvre-Lens.",
  ecosystemEn:
    "Noyelles-sous-Lens est situee dans l'agglomeration de Lens (3 km), au coeur du bassin minier. Tissu economique compose de TPE de services, PME commerciales, artisans BTP, professions liberales et activites tertiaires liees a la proximite du Louvre-Lens.",
  distancesFr:
    "Gare de Lens a 5 min. Lille Europe (TGV) a 45 min en TER. Aeroport Lille-Lesquin a 50 min. Paris-Nord a 1h10 via TGV Lens.",
  distancesEn:
    "Gare de Lens a 5 min. Lille Europe (TGV) a 45 min en TER. Aeroport Lille-Lesquin a 50 min. Paris-Nord a 1h10 via TGV Lens.",
  topSectorsNaf: [
    "Commerce de proximite",
    "TPE services aux entreprises",
    "Artisanat BTP",
    "Professions liberales",
    "Activites tertiaires",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA a Noyelles-sous-Lens : diagnostic adapte aux TPE de services, PME commerciales et artisans du bassin minier lensois. ROI chiffre, audit Flash 490 EUR HT.",
      en: "Audit IA a Noyelles-sous-Lens : diagnostic adapte aux TPE de services, PME commerciales et artisans du bassin minier lensois. ROI chiffre, audit Flash 490 EUR HT.",
    },
    interventions: {
      fr: "Interventions IA sur site a Noyelles-sous-Lens : formations adaptees aux commerces, bureaux tertiaires, cabinets et ateliers artisanaux. Equipes autonomes sur outils IA.",
      en: "Interventions IA sur site a Noyelles-sous-Lens : formations adaptees aux commerces, bureaux tertiaires, cabinets et ateliers artisanaux. Equipes autonomes sur outils IA.",
    },
    implementation: {
      fr: "Implementation IA a Noyelles-sous-Lens : automatisation administrative, agents conversationnels clients, CRM augmentes pour PME de l'agglomeration lensoise.",
      en: "Implementation IA a Noyelles-sous-Lens : automatisation administrative, agents conversationnels clients, CRM augmentes pour PME de l'agglomeration lensoise.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 a Noyelles-sous-Lens : accompagnement personnalise dirigeants TPE, gerants PME et professions liberales du bassin lensois.",
      en: "Coaching 1-to-1 a Noyelles-sous-Lens : accompagnement personnalise dirigeants TPE, gerants PME et professions liberales du bassin lensois.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il a Noyelles-sous-Lens et dans le Lensois ?",
      a: "Oui, Axion-IA intervient a Noyelles-sous-Lens et dans toute l'agglomeration lensoise (Lens, Lievin, Henin-Beaumont). Consultants depuis Lille (45 min TER) ou Paris (TGV 1h10 via Lens).",
    },
    {
      q: "Quels services IA pour les commercants de Noyelles-sous-Lens ?",
      a: "Pour les commercants du bassin lensois, nous proposons : gestion stocks IA, agents conversationnels, fidelisation client automatisee, comptabilite augmentee. Audit Flash 490 EUR HT.",
    },
    {
      q: "Quel est le tarif d'un audit IA a Noyelles-sous-Lens ?",
      a: "L'audit Flash demarre a 490 EUR HT. Tous nos tarifs sont publics et consultables sur notre site. Intervention sur site sous 5 jours ouvres apres validation.",
    },
    {
      q: "Accompagnez-vous les professions liberales de Noyelles-sous-Lens ?",
      a: "Oui, nous accompagnons avocats, experts-comptables, medecins et autres professions liberales du Lensois : automatisation documents, IA redaction, gestion agenda intelligente.",
    },
    {
      q: "Vos interventions respectent-elles le RGPD a Noyelles-sous-Lens ?",
      a: "Oui, toutes nos interventions a Noyelles-sous-Lens sont conformes au RGPD. Protection des donnees de votre entreprise et de vos clients garantie contractuellement.",
    },
  ],
};
