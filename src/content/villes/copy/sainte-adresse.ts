// AUTO-GENERATED 2026-05-27 — T4 batch #23 Claude Code (Will).
// Sainte-Adresse (76, Seine-Maritime) — Le Havre, Nice havrais, residentielle prestige, 7 004 hab.

import type { VilleCopy } from "./types";

export const SAINTE_ADRESSE_COPY: VilleCopy = {
  pitchFr:
    "Sainte-Adresse (76), commune residentielle de prestige de l'agglomeration havraise en Seine-Maritime, reunit pres de 7 000 habitants. Surnommee le Nice havrais pour ses vues sur mer, elle accueille des professions liberales, des TPE de services, des commerces et des acteurs du tourisme. Axion-IA y intervient pour tous types d'activites.",
  pitchEn:
    "Sainte-Adresse (76), commune residentielle de prestige de l'agglomeration havraise en Seine-Maritime, reunit pres de 7 000 habitants. Surnommee le Nice havrais pour ses vues sur mer, elle accueille des professions liberales, des TPE de services, des commerces et des acteurs du tourisme. Axion-IA y intervient pour tous types d'activites.",
  seoHook: "prestige littoral, professions liberales & tourisme",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient a Sainte-Adresse (76). Nous accompagnons les TPE, PME et professions liberales du Nice havrais : medecins, avocats, commercants, acteurs du tourisme et prestataires de services. Audit Flash a partir de {{price:audit-flash|flat}}.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient a Sainte-Adresse (76). Nous accompagnons les TPE, PME et professions liberales du Nice havrais : medecins, avocats, commercants, acteurs du tourisme et prestataires de services. Audit Flash a partir de {{price:audit-flash|flat}}.",
  ecosystemFr:
    "Sainte-Adresse est la commune residentielle de prestige du Havre, surnommee le Nice havrais pour son front de mer et ses villas. Tissu economique domine par les professions liberales, TPE de services, commerces de proximite, hebergements touristiques et artisanat. Acces direct Le Havre centre.",
  ecosystemEn:
    "Sainte-Adresse est la commune residentielle de prestige du Havre, surnommee le Nice havrais pour son front de mer et ses villas. Tissu economique domine par les professions liberales, TPE de services, commerces de proximite, hebergements touristiques et artisanat. Acces direct Le Havre centre.",
  distancesFr:
    "Le Havre centre a 5 min. Gare du Havre a 10 min (TGV Paris 2h10). A29 Le Havre-Amiens a 15 min. Rouen a 1h par A150. Paris a 2h15 par A13. Etretat a 30 min.",
  distancesEn:
    "Le Havre centre a 5 min. Gare du Havre a 10 min (TGV Paris 2h10). A29 Le Havre-Amiens a 15 min. Rouen a 1h par A150. Paris a 2h15 par A13. Etretat a 30 min.",
  topSectorsNaf: [
    "Professions liberales & sante",
    "Tourisme & hebergement",
    "Commerce de proximite",
    "TPE services aux entreprises",
    "Artisanat & renovation",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA a Sainte-Adresse : diagnostic adapte aux professions liberales, TPE de services et acteurs du tourisme du Nice havrais. ROI chiffre, audit Flash {{price:audit-flash|flat}}.",
      en: "Audit IA a Sainte-Adresse : diagnostic adapte aux professions liberales, TPE de services et acteurs du tourisme du Nice havrais. ROI chiffre, audit Flash {{price:audit-flash|flat}}.",
    },
    interventions: {
      fr: "Interventions IA sur site a Sainte-Adresse : formations adaptees aux cabinets liberaux, hebergeurs touristiques, commerces et TPE de services. Equipes autonomes sur outils IA.",
      en: "Interventions IA sur site a Sainte-Adresse : formations adaptees aux cabinets liberaux, hebergeurs touristiques, commerces et TPE de services. Equipes autonomes sur outils IA.",
    },
    implementation: {
      fr: "Implementation IA a Sainte-Adresse : automatisation documents juridiques/comptables, agents IA reservations tourisme, CRM augmentes pour professions liberales et commercants.",
      en: "Implementation IA a Sainte-Adresse : automatisation documents juridiques/comptables, agents IA reservations tourisme, CRM augmentes pour professions liberales et commercants.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 a Sainte-Adresse : accompagnement personnalise medecins, avocats, gerants hebergements et dirigeants TPE du Nice havrais.",
      en: "Coaching 1-to-1 a Sainte-Adresse : accompagnement personnalise medecins, avocats, gerants hebergements et dirigeants TPE du Nice havrais.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il a Sainte-Adresse et dans l'agglo du Havre ?",
      a: "Oui, Axion-IA intervient a Sainte-Adresse et dans toute l'agglomeration havraise (Le Havre, Harfleur, Octeville-sur-Mer, Gonfreville-l'Orcher). Consultants depuis Le Havre (5 min) ou Paris (2h15).",
    },
    {
      q: "Quels services IA pour les professions liberales de Sainte-Adresse ?",
      a: "Pour les professions liberales, nous proposons : automatisation documents, IA redaction, gestion agenda intelligente, accueil patients/clients IA. Audit Flash {{price:audit-flash|flat}}.",
    },
    {
      q: "Quel est le tarif d'un audit IA a Sainte-Adresse ?",
      a: "L'audit Flash demarre a {{price:audit-flash|flat}}. Tous nos tarifs sont publics et consultables sur notre site. Intervention sur site sous 5 jours ouvres apres validation.",
    },
    {
      q: "Accompagnez-vous les hebergeurs touristiques de Sainte-Adresse ?",
      a: "Oui, nous accompagnons les hebergeurs touristiques de Sainte-Adresse : automatisation reservations, agents conversationnels multilingues, gestion avis clients IA, revenue management.",
    },
    {
      q: "Vos interventions respectent-elles le RGPD a Sainte-Adresse ?",
      a: "Oui, toutes nos interventions a Sainte-Adresse sont conformes au RGPD. Protection des donnees de votre cabinet/entreprise et de vos clients garantie contractuellement.",
    },
  ],
};
