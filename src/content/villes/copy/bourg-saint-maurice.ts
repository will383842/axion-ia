// AUTO-GENERATED 2026-05-27 — T4 batch #19 Claude Code (Will).
// Bourg-Saint-Maurice (73, Savoie) — Haute-Tarentaise, station Les Arcs à proximité, sports montagne majeur, funiculaire Arc 1600, TGV direct Paris hiver.

import type { VilleCopy } from "./types";

export const BOURG_SAINT_MAURICE_COPY: VilleCopy = {
  pitchFr:
    "Bourg-Saint-Maurice (73), commune de Haute-Tarentaise en Savoie, est la porte d'accès à la station Les Arcs et au domaine Paradiski. Sports de montagne majeurs, funiculaire vers Arc 1600, gare SNCF TGV direct Paris (hiver). Tissu tourisme sports d'hiver, hôtellerie montagne, remontées mécaniques, TPE commerces station, services saisonniers. Axion-IA y intervient pour tous types d'activités.",
  pitchEn:
    "Bourg-Saint-Maurice (73), commune de Haute-Tarentaise en Savoie, est la porte d'accès à la station Les Arcs et au domaine Paradiski. Sports de montagne majeurs, funiculaire vers Arc 1600, gare SNCF TGV direct Paris (hiver). Tissu tourisme sports d'hiver, hôtellerie montagne, remontées mécaniques, TPE commerces station, services saisonniers. Axion-IA y intervient pour tous types d'activités.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Bourg-Saint-Maurice (73). Nous accompagnons TPE et PME de la Haute-Tarentaise : opérateurs touristiques Les Arcs/Paradiski, hôteliers et résidences de montagne, exploitants remontées mécaniques, commerçants de station, prestataires saisonniers et services aux entreprises.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Bourg-Saint-Maurice (73). Nous accompagnons TPE et PME de la Haute-Tarentaise : opérateurs touristiques Les Arcs/Paradiski, hôteliers et résidences de montagne, exploitants remontées mécaniques, commerçants de station, prestataires saisonniers et services aux entreprises.",
  ecosystemFr:
    "Bourg-Saint-Maurice est la commune centre de la Haute-Tarentaise (7 300 habitants), porte d'accès à la station Les Arcs et au domaine Paradiski (425 km de pistes). Le funiculaire relie la ville à Arc 1600. Gare SNCF avec TGV direct Paris en saison d'hiver. Économie dominée par le tourisme de sports d'hiver, l'hôtellerie de montagne, les remontées mécaniques et les services saisonniers.",
  ecosystemEn:
    "Bourg-Saint-Maurice est la commune centre de la Haute-Tarentaise (7 300 habitants), porte d'accès à la station Les Arcs et au domaine Paradiski (425 km de pistes). Le funiculaire relie la ville à Arc 1600. Gare SNCF avec TGV direct Paris en saison d'hiver. Économie dominée par le tourisme de sports d'hiver, l'hôtellerie de montagne, les remontées mécaniques et les services saisonniers.",
  distancesFr:
    "Albertville à 45 min par N90. Chambéry à 1h30. Aéroport Lyon-Saint-Exupéry à 2h30. Gare SNCF Bourg-Saint-Maurice (TGV direct Paris en hiver). Funiculaire Arc 1600 à 7 min.",
  distancesEn:
    "Albertville à 45 min par N90. Chambéry à 1h30. Aéroport Lyon-Saint-Exupéry à 2h30. Gare SNCF Bourg-Saint-Maurice (TGV direct Paris en hiver). Funiculaire Arc 1600 à 7 min.",
  topSectorsNaf: [
    "Tourisme sports d'hiver (Les Arcs / Paradiski)",
    "Hôtellerie & résidences de montagne",
    "Remontées mécaniques & domaines skiables",
    "TPE commerces de station",
    "Services saisonniers",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Bourg-Saint-Maurice : diagnostic adapté aux opérateurs touristiques Les Arcs/Paradiski, hôteliers montagne, exploitants remontées mécaniques, TPE commerces station et prestataires saisonniers. ROI chiffré, audit Flash 490 € HT.",
      en: "Audit IA à Bourg-Saint-Maurice : diagnostic adapté aux opérateurs touristiques Les Arcs/Paradiski, hôteliers montagne, exploitants remontées mécaniques, TPE commerces station et prestataires saisonniers. ROI chiffré, audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Bourg-Saint-Maurice : ateliers dans vos locaux hôteliers, bureau exploitation ou commerce de station. Vos équipes repartent autonomes sur outils IA configurés métier tourisme et montagne.",
      en: "Interventions IA sur site à Bourg-Saint-Maurice : ateliers dans vos locaux hôteliers, bureau exploitation ou commerce de station. Vos équipes repartent autonomes sur outils IA configurés métier tourisme et montagne.",
    },
    implementation: {
      fr: "Implémentation IA à Bourg-Saint-Maurice : agents conversationnels multilingues réservation ski, prédiction affluence domaine skiable, automatisation check-in hôtelier, tarification dynamique saisonnière, maintenance prédictive remontées mécaniques.",
      en: "Implémentation IA à Bourg-Saint-Maurice : agents conversationnels multilingues réservation ski, prédiction affluence domaine skiable, automatisation check-in hôtelier, tarification dynamique saisonnière, maintenance prédictive remontées mécaniques.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Bourg-Saint-Maurice : sessions dans vos locaux station, hôtel ou en visio. Dirigeants hôtellerie montagne, exploitants remontées, gérants TPE commerces station, prestataires saisonniers Haute-Tarentaise.",
      en: "Coaching 1-to-1 à Bourg-Saint-Maurice : sessions dans vos locaux station, hôtel ou en visio. Dirigeants hôtellerie montagne, exploitants remontées, gérants TPE commerces station, prestataires saisonniers Haute-Tarentaise.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Bourg-Saint-Maurice et en Haute-Tarentaise ?",
      a: "Oui, Axion-IA intervient à Bourg-Saint-Maurice et dans toute la Haute-Tarentaise (Les Arcs, La Rosière, Sainte-Foy-Tarentaise, Séez, Villaroger). Couverture étendue jusqu'à Tignes et Val-d'Isère.",
    },
    {
      q: "Travaillez-vous avec les opérateurs touristiques Les Arcs et Paradiski ?",
      a: "Oui, nous accompagnons les opérateurs touristiques Les Arcs/Paradiski sur leurs cas IA : agents conversationnels multilingues réservation, prédiction affluence pistes, tarification dynamique forfaits, automatisation check-in, analyse avis clients multi-langues.",
    },
    {
      q: "Accompagnez-vous les hôteliers et résidences de montagne ?",
      a: "Oui, nous accompagnons les hôteliers et résidences de montagne à Bourg-Saint-Maurice et Les Arcs : automatisation check-in/check-out, chatbots conciergerie multilingues, revenue management IA, gestion saisonnalité, classification avis clients.",
    },
    {
      q: "Quels tarifs pour les TPE commerces de station et saisonniers ?",
      a: "Pour les TPE commerces de station et prestataires saisonniers à Bourg-Saint-Maurice, audit Flash 490 € HT, intervention 1 journée formation, coaching 1-to-1. Cas typiques : gestion stocks saison, automatisation location matériel, agents IA réservation.",
    },
    {
      q: "Comment se déroule une mission à Bourg-Saint-Maurice ?",
      a: "Nos consultants se déplacent à Bourg-Saint-Maurice (accessible TGV direct Paris en hiver, 45 min depuis Albertville). Interventions possibles en station ou en vallée. Pour les missions multi-jours, hébergement local. Visio possible hors saison.",
    },
  ],
};
