// AUTO-GENERATED 2026-05-27 — T4 batch #22 Claude Code (Will).
// Remiremont (88, Vosges) — Sous-préfecture, abbaye, bords de Moselle, 7 388 hab.

import type { VilleCopy } from "./types";

export const REMIREMONT_COPY: VilleCopy = {
  pitchFr:
    "Remiremont (88), sous-préfecture des Vosges, est une ancienne ville abbatiale au cœur du massif vosgien sud, traversée par la Moselle. Tissu de TPE/PME administration, commerce de centre-ville sous arcades, hôtellerie-restauration, tourisme patrimonial et thermal, artisanat textile et services aux entreprises. Axion-IA y intervient pour tous types d'activités.",
  pitchEn:
    "Remiremont (88), sous-préfecture des Vosges, est une ancienne ville abbatiale au cœur du massif vosgien sud, traversée par la Moselle. Tissu de TPE/PME administration, commerce de centre-ville sous arcades, hôtellerie-restauration, tourisme patrimonial et thermal, artisanat textile et services aux entreprises. Axion-IA y intervient pour tous types d'activités.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Remiremont (88). Nous accompagnons TPE et PME : commerçants de centre-ville, hôteliers-restaurateurs, professionnels du tourisme patrimonial, artisans textiles, professions libérales et prestataires de services du sud-vosgien. Audit Flash dès 490 € HT.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Remiremont (88). Nous accompagnons TPE et PME : commerçants de centre-ville, hôteliers-restaurateurs, professionnels du tourisme patrimonial, artisans textiles, professions libérales et prestataires de services du sud-vosgien. Audit Flash dès 490 € HT.",
  ecosystemFr:
    "Remiremont est une sous-préfecture des Vosges située au confluent de la Moselle et de la Moselotte, au pied du massif vosgien sud. La ville historique abbatiale conserve son abbaye, sa basilique Saint-Pierre et ses célèbres arcades commerçantes du centre-ville. L'économie locale repose sur l'administration sous-préfectorale, le commerce de centre-ville (arcades), l'hôtellerie-restauration, le tourisme patrimonial et thermal (Plombières-les-Bains à proximité), l'artisanat textile, les professions libérales et les services aux entreprises.",
  ecosystemEn:
    "Remiremont est une sous-préfecture des Vosges située au confluent de la Moselle et de la Moselotte, au pied du massif vosgien sud. La ville historique abbatiale conserve son abbaye, sa basilique Saint-Pierre et ses célèbres arcades commerçantes du centre-ville. L'économie locale repose sur l'administration sous-préfectorale, le commerce de centre-ville (arcades), l'hôtellerie-restauration, le tourisme patrimonial et thermal (Plombières-les-Bains à proximité), l'artisanat textile, les professions libérales et les services aux entreprises.",
  distancesFr:
    "Gare SNCF Remiremont (TER Épinal-Mulhouse). Épinal à 30 min. Nancy à 1h20. Mulhouse à 1h. Belfort à 50 min. Strasbourg à 2h. Aéroport Bâle-Mulhouse à 1h15.",
  distancesEn:
    "Gare SNCF Remiremont (TER Épinal-Mulhouse). Épinal à 30 min. Nancy à 1h20. Mulhouse à 1h. Belfort à 50 min. Strasbourg à 2h. Aéroport Bâle-Mulhouse à 1h15.",
  topSectorsNaf: [
    "Administration & services publics",
    "Commerce de centre-ville (arcades) & artisanat",
    "Hôtellerie-restauration & tourisme patrimonial",
    "Industrie textile & sous-traitance",
    "Professions libérales & services aux entreprises",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Remiremont : diagnostic adapté aux commerçants de centre-ville, hôteliers-restaurateurs, professionnels du tourisme patrimonial, artisans textiles, professions libérales et prestataires de services du sud-vosgien. Audit Flash 490 € HT, ROI chiffré.",
      en: "Audit IA à Remiremont : diagnostic adapté aux commerçants de centre-ville, hôteliers-restaurateurs, professionnels du tourisme patrimonial, artisans textiles, professions libérales et prestataires de services du sud-vosgien. Audit Flash 490 € HT, ROI chiffré.",
    },
    interventions: {
      fr: "Interventions IA sur site à Remiremont : formats adaptés au commerce des arcades, à l'hôtel-restaurant, au cabinet libéral, à l'atelier textile ou à la TPE de services. Vos équipes repartent autonomes sur outils IA configurés métier.",
      en: "Interventions IA sur site à Remiremont : formats adaptés au commerce des arcades, à l'hôtel-restaurant, au cabinet libéral, à l'atelier textile ou à la TPE de services. Vos équipes repartent autonomes sur outils IA configurés métier.",
    },
    implementation: {
      fr: "Implémentation IA à Remiremont : automatisation CRM commerce de centre-ville, réservations hôtelières, gestion cabinets libéraux, e-commerce textile, agents conversationnels et automatisations administratives pour PME sud-vosgiennes.",
      en: "Implémentation IA à Remiremont : automatisation CRM commerce de centre-ville, réservations hôtelières, gestion cabinets libéraux, e-commerce textile, agents conversationnels et automatisations administratives pour PME sud-vosgiennes.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Remiremont : sessions dans votre commerce, hôtel, cabinet, atelier ou en visio. Dirigeants TPE/PME, artisans et professions libérales du sud-vosgien souhaitant maîtriser l'IA au quotidien.",
      en: "Coaching 1-to-1 à Remiremont : sessions dans votre commerce, hôtel, cabinet, atelier ou en visio. Dirigeants TPE/PME, artisans et professions libérales du sud-vosgien souhaitant maîtriser l'IA au quotidien.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Remiremont et dans le sud-vosgien ?",
      a: "Oui, Axion-IA intervient à Remiremont et sur tout le sud des Vosges (Plombières-les-Bains, Le Thillot, Saint-Nabord, La Bresse, Gérardmer). Accès direct par TER ligne Épinal-Mulhouse.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Remiremont ?",
      a: "L'Audit Flash démarre à 490 € HT pour 4 heures de diagnostic. Adapté aux TPE et PME de Remiremont et du sud-vosgien, il identifie les cas d'usage IA à fort ROI pour votre commerce, hôtel, cabinet ou activité de services.",
    },
    {
      q: "Accompagnez-vous les commerçants des arcades de Remiremont ?",
      a: "Oui, nous accompagnons les commerçants du centre-ville historique de Remiremont (célèbres arcades). Cas IA typiques : gestion stocks intelligente, fidélisation client, communication réseaux sociaux IA, chatbot accueil, prévision de fréquentation touristique et e-commerce.",
    },
    {
      q: "L'IA peut-elle aider les professions libérales de Remiremont ?",
      a: "Oui, nous accompagnons avocats, experts-comptables, médecins, kinés et notaires de Remiremont. Cas concrets : automatisation prise de rendez-vous, rédaction assistée par IA, gestion documentaire intelligente, relances clients, facturation et veille réglementaire.",
    },
    {
      q: "Proposez-vous des formations IA à Remiremont ?",
      a: "Oui, coaching 1-to-1 pour dirigeants et cadres à Remiremont, à partir de 990 € HT. Sessions dans votre commerce, hôtel, cabinet, atelier ou en visio. Approche terrain adaptée au tissu commercial, touristique et libéral sud-vosgien.",
    },
  ],
};
