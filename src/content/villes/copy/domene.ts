// AUTO-GENERATED 2026-05-27 — T4 batch #4 Claude Code (Will).
// Domène (38, Isère) — vallée du Grésivaudan, banlieue est Grenoble, Grenoble-Alpes Métropole.

import type { VilleCopy } from "./types";

export const DOMENE_COPY: VilleCopy = {
  pitchFr:
    "Domène (38), commune de la vallée du Grésivaudan à l'est immédiat de Grenoble, est intégrée à Grenoble-Alpes Métropole. Tissu PME tertiaires et industrielles, TPE artisans BTP, commerces, professions libérales. Axion-IA y intervient pour tous types d'activités domenoises du Grésivaudan.",
  pitchEn:
    "Domène (38), commune de la vallée du Grésivaudan à l'est immédiat de Grenoble, est intégrée à Grenoble-Alpes Métropole. Tissu PME tertiaires et industrielles, TPE artisans BTP, commerces, professions libérales. Axion-IA y intervient pour tous types d'activités domenoises du Grésivaudan.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Domène (38). Nous accompagnons tous types d'activités : PME tertiaires et industrielles du Grésivaudan, TPE artisans BTP, commerçants, professions libérales, indépendants et services aux entreprises de Grenoble-Alpes Métropole en zone est.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Domène (38). Nous accompagnons tous types d'activités : PME tertiaires et industrielles du Grésivaudan, TPE artisans BTP, commerçants, professions libérales, indépendants et services aux entreprises de Grenoble-Alpes Métropole en zone est.",
  ecosystemFr:
    "Domène est membre de Grenoble-Alpes Métropole (« La Métro ») dans la vallée du Grésivaudan. Position péri-urbaine immédiate de Grenoble, à l'est. Tissu mixte : PME tertiaires et industrielles, TPE artisanat BTP, commerces de proximité, professions libérales. Accès rapide vers parc technologique Inovallée Meylan (5 min) et campus universitaire est.",
  ecosystemEn:
    "Domène est membre de Grenoble-Alpes Métropole (« La Métro ») dans la vallée du Grésivaudan. Position péri-urbaine immédiate de Grenoble, à l'est. Tissu mixte : PME tertiaires et industrielles, TPE artisanat BTP, commerces de proximité, professions libérales. Accès rapide vers parc technologique Inovallée Meylan (5 min) et campus universitaire est.",
  distancesFr:
    "Grenoble centre à 15 min voiture A41. Meylan (Inovallée) à 5 min. Aéroport Grenoble-Alpes-Isère à 1h. Chambéry à 50 min A41. Axe A41 vers vallée Grésivaudan / Chambéry.",
  distancesEn:
    "Grenoble centre à 15 min voiture A41. Meylan (Inovallée) à 5 min. Aéroport Grenoble-Alpes-Isère à 1h. Chambéry à 50 min A41. Axe A41 vers vallée Grésivaudan / Chambéry.",
  topSectorsNaf: [
    "PME tertiaires & services aux entreprises",
    "Industrie périphérique Grésivaudan",
    "TPE artisanat BTP",
    "Commerce de proximité",
    "Professions libérales (santé, juridique)",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Domène : diagnostic adapté aux PME tertiaires/industrielles, TPE artisans, professions libérales du bassin grenoblois est. ROI chiffré, audit Flash 490 € HT accessible.",
      en: "Audit IA à Domène : diagnostic adapté aux PME tertiaires/industrielles, TPE artisans, professions libérales du bassin grenoblois est. ROI chiffré, audit Flash 490 € HT accessible.",
    },
    interventions: {
      fr: "Interventions IA sur site à Domène : formats adaptés au bureau PME, atelier industriel, cabinet professionnel. Vos équipes repartent autonomes sur outils IA configurés métier.",
      en: "Interventions IA sur site à Domène : formats adaptés au bureau PME, atelier industriel, cabinet professionnel. Vos équipes repartent autonomes sur outils IA configurés métier.",
    },
    implementation: {
      fr: "Implémentation IA à Domène : agents conversationnels, automatisation workflow, classification documents, prédiction maintenance industrielle, CRM augmentés. Tissu Grésivaudan / Métro.",
      en: "Implémentation IA à Domène : agents conversationnels, automatisation workflow, classification documents, prédiction maintenance industrielle, CRM augmentés. Tissu Grésivaudan / Métro.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Domène : sessions dans votre bureau, atelier, cabinet ou en visio. Dirigeants PME, médecins, avocats, artisans, gérants TPE domenois souhaitant intégrer l'IA.",
      en: "Coaching 1-to-1 à Domène : sessions dans votre bureau, atelier, cabinet ou en visio. Dirigeants PME, médecins, avocats, artisans, gérants TPE domenois souhaitant intégrer l'IA.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Domène et est grenoblois ?",
      a: "Oui, Axion-IA intervient à Domène et tout l'est grenoblois (Meylan, Saint-Ismier, Gières, Villard-Bonnot, Crolles). Couverture Grenoble-Alpes Métropole et vallée du Grésivaudan.",
    },
    {
      q: "Accompagnez-vous les PME tertiaires et industrielles domenoises ?",
      a: "Oui, nous accompagnons les PME tertiaires/industrielles à Domène sur leurs cas IA : agents conversationnels, automatisation workflow, classification documents, prédiction maintenance. Proximité immédiate Inovallée Meylan pour collaborations tech.",
    },
    {
      q: "Travaillez-vous avec les TPE artisans BTP ?",
      a: "Oui, nous accompagnons les TPE artisans BTP à Domène (plombiers, électriciens, menuisiers) sur des cas IA pratiques : automatisation devis, agents IA prise de RDV, classification photos chantier, gestion stocks. Audit Flash 490 € HT adapté.",
    },
    {
      q: "Quels services pour les professions libérales ?",
      a: "Pour les professions libérales à Domène (médecins, avocats, expert-comptables), audit Flash 490 € HT, intervention 1 journée formation, coaching 1-to-1. Cas IA : agents conversationnels patients/clients, automatisation gestion RDV, classification dossiers. RGPD strict.",
    },
    {
      q: "Comment se passe une mission depuis Grenoble ?",
      a: "Nos consultants viennent de Grenoble (15 min A41). Pour les multi-jours, hébergement local possible. Frais de déplacement facturés à part du forfait, tarifs publics.",
    },
  ],
};
