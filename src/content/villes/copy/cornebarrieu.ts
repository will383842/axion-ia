// AUTO-GENERATED 2026-05-27 — T4 batch #24 Claude Code (Will).
// Cornebarrieu (31, Haute-Garonne) — Aéroport Toulouse-Blagnac, aéronautique.

import type { VilleCopy } from "./types";

export const CORNEBARRIEU_COPY: VilleCopy = {
  pitchFr:
    "Cornebarrieu (31), commune à proximité immédiate de l'aéroport Toulouse-Blagnac, près de 9 000 habitants. Tissu TPE et PME aéronautiques, sous-traitants industriels, services aux entreprises, professions libérales, commerces. Axion-IA y intervient pour tous types d'activités du pôle aéroportuaire toulousain.",
  pitchEn:
    "Cornebarrieu (31), commune à proximité immédiate de l'aéroport Toulouse-Blagnac, près de 9 000 habitants. Tissu TPE et PME aéronautiques, sous-traitants industriels, services aux entreprises, professions libérales, commerces. Axion-IA y intervient pour tous types d'activités du pôle aéroportuaire toulousain.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Cornebarrieu (31). Nous accompagnons TPE et PME aéronautiques, sous-traitants industriels, professions libérales et commerçants du pôle aéroportuaire Toulouse. Audit Flash dès 490 € HT.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Cornebarrieu (31). Nous accompagnons TPE et PME aéronautiques, sous-traitants industriels, professions libérales et commerçants du pôle aéroportuaire Toulouse. Audit Flash dès 490 € HT.",
  ecosystemFr:
    "Cornebarrieu est une commune stratégique du pôle aéroportuaire toulousain. PME aéronautiques et sous-traitants, TPE de services aux entreprises, zone Aéroconstellation, professions libérales, commerces. Proximité immédiate de l'aéroport Toulouse-Blagnac et de Blagnac.",
  ecosystemEn:
    "Cornebarrieu est une commune stratégique du pôle aéroportuaire toulousain. PME aéronautiques et sous-traitants, TPE de services aux entreprises, zone Aéroconstellation, professions libérales, commerces. Proximité immédiate de l'aéroport Toulouse-Blagnac et de Blagnac.",
  distancesFr:
    "Toulouse centre à 20 min (D902). Aéroport Toulouse-Blagnac à 5 min. Gare Matabiau à 25 min. Blagnac à 5 min. Beauzelle à 8 min.",
  distancesEn:
    "Toulouse centre à 20 min (D902). Aéroport Toulouse-Blagnac à 5 min. Gare Matabiau à 25 min. Blagnac à 5 min. Beauzelle à 8 min.",
  topSectorsNaf: [
    "Aéronautique & sous-traitance",
    "Services aux entreprises",
    "Programmation informatique",
    "Professions libérales & santé",
    "Commerce de détail",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Cornebarrieu : diagnostic adapté aux PME aéronautiques, sous-traitants industriels et TPE de services du pôle aéroportuaire. Audit Flash 490 € HT.",
      en: "Audit IA à Cornebarrieu : diagnostic adapté aux PME aéronautiques, sous-traitants industriels et TPE de services du pôle aéroportuaire. Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Cornebarrieu : formats adaptés au sous-traitant aéronautique, à la PME tech, au cabinet de conseil. Vos équipes repartent autonomes.",
      en: "Interventions IA sur site à Cornebarrieu : formats adaptés au sous-traitant aéronautique, à la PME tech, au cabinet de conseil. Vos équipes repartent autonomes.",
    },
    implementation: {
      fr: "Implémentation IA à Cornebarrieu : automatisation contrôle qualité aéronautique, gestion documentation technique, agents conversationnels, optimisation supply chain.",
      en: "Implémentation IA à Cornebarrieu : automatisation contrôle qualité aéronautique, gestion documentation technique, agents conversationnels, optimisation supply chain.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Cornebarrieu : sessions dirigeants PME aéronautiques, gérants sous-traitants, cadres du pôle aéroportuaire toulousain.",
      en: "Coaching 1-to-1 à Cornebarrieu : sessions dirigeants PME aéronautiques, gérants sous-traitants, cadres du pôle aéroportuaire toulousain.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Cornebarrieu ?",
      a: "Oui, Axion-IA intervient à Cornebarrieu et dans tout le pôle aéroportuaire (Blagnac, Beauzelle, Aussonne). Nos consultants se déplacent en 20 min depuis Toulouse.",
    },
    {
      q: "Quels secteurs accompagnez-vous à Cornebarrieu ?",
      a: "Nous accompagnons tous types d'activités : PME aéronautiques, sous-traitants, TPE de services, professions libérales, commerces. Aucun secteur exclu.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Cornebarrieu ?",
      a: "L'Audit Flash démarre à 490 € HT pour une TPE. Pour les PME aéronautiques, audit approfondi entre 1 900 et 3 900 € HT. Tarifs publics.",
    },
    {
      q: "L'IA peut-elle aider un sous-traitant aéronautique à Cornebarrieu ?",
      a: "Oui, l'IA optimise contrôle qualité, documentation technique, traçabilité pièces, planification production et reporting client. Gains mesurables rapidement.",
    },
    {
      q: "Comment se déroule une intervention IA à Cornebarrieu ?",
      a: "Nos consultants se déplacent à Cornebarrieu (20 min depuis Toulouse, 5 min de l'aéroport). Intervention 1 journée minimum, atelier pratique.",
    },
  ],
};
