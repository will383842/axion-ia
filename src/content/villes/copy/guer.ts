// AUTO-GENERATED 2026-05-27 — T4 batch #21 Claude Code (Will).
// Guer (56, Morbihan) — Camp de Coëtquidan (Saint-Cyr), militaire.

import type { VilleCopy } from "./types";

export const GUER_COPY: VilleCopy = {
  pitchFr:
    "Guer (56), commune du Morbihan intérieur marquée par le camp de Coëtquidan et les écoles militaires de Saint-Cyr, associe activités liées à la défense, TPE artisanales, PME de services et commerces de bourg. Axion-IA couvre l'ensemble des activités professionnelles présentes.",
  pitchEn:
    "Guer (56), commune du Morbihan intérieur marquée par le camp de Coëtquidan et les écoles militaires de Saint-Cyr, associe activités liées à la défense, TPE artisanales, PME de services et commerces de bourg. Axion-IA couvre l'ensemble des activités professionnelles présentes.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Guer (56). Nous accompagnons tous types d'activités : TPE artisanales, PME de services et de sous-traitance, commerces de bourg, professions libérales, prestataires du secteur défense et acteurs économiques du Morbihan intérieur.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Guer (56). Nous accompagnons tous types d'activités : TPE artisanales, PME de services et de sous-traitance, commerces de bourg, professions libérales, prestataires du secteur défense et acteurs économiques du Morbihan intérieur.",
  ecosystemFr:
    "Guer est une commune du Morbihan intérieur dont l'économie est structurée par la présence du camp militaire de Coëtquidan et des écoles de Saint-Cyr. Ce pôle défense génère un écosystème de sous-traitants, prestataires de services et commerces de proximité. L'artisanat, l'agriculture et les professions libérales complètent le tissu économique local.",
  ecosystemEn:
    "Guer est une commune du Morbihan intérieur dont l'économie est structurée par la présence du camp militaire de Coëtquidan et des écoles de Saint-Cyr. Ce pôle défense génère un écosystème de sous-traitants, prestataires de services et commerces de proximité. L'artisanat, l'agriculture et les professions libérales complètent le tissu économique local.",
  distancesFr:
    "Ploërmel à 20 min par D724. Vannes à 45 min. Rennes à 55 min par N24. Gare de Guer à 5 min (TER Rennes-Vannes). Redon à 40 min. Aéroport Rennes-Bretagne à 65 min.",
  distancesEn:
    "Ploërmel à 20 min par D724. Vannes à 45 min. Rennes à 55 min par N24. Gare de Guer à 5 min (TER Rennes-Vannes). Redon à 40 min. Aéroport Rennes-Bretagne à 65 min.",
  seoHook: "défense, artisanat & services Morbihan intérieur",
  topSectorsNaf: [
    "Services & sous-traitance secteur défense",
    "TPE artisanat & services à la personne",
    "Commerce de détail & proximité",
    "Professions libérales & santé",
    "Agriculture & élevage",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Guer : diagnostic adapté aux TPE artisanales, PME de services, prestataires du secteur défense et commerces du Morbihan intérieur. ROI mesurable, audit Flash dès 490 € HT accessible à toute structure.",
      en: "Audit IA à Guer : diagnostic adapté aux TPE artisanales, PME de services, prestataires du secteur défense et commerces du Morbihan intérieur. ROI mesurable, audit Flash dès 490 € HT accessible à toute structure.",
    },
    interventions: {
      fr: "Interventions IA sur site à Guer : formats adaptés au prestataire de services, au commerce de bourg ou au bureau PME. Vos équipes repartent autonomes sur les outils IA métier adaptés à votre activité.",
      en: "Interventions IA sur site à Guer : formats adaptés au prestataire de services, au commerce de bourg ou au bureau PME. Vos équipes repartent autonomes sur les outils IA métier adaptés à votre activité.",
    },
    implementation: {
      fr: "Implémentation IA à Guer : automatisation gestion documentaire, agents conversationnels relation client, chatbots prise de RDV, classification courriers et dossiers. Adapté tissu économique Morbihan intérieur et écosystème défense.",
      en: "Implémentation IA à Guer : automatisation gestion documentaire, agents conversationnels relation client, chatbots prise de RDV, classification courriers et dossiers. Adapté tissu économique Morbihan intérieur et écosystème défense.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Guer : sessions dans vos locaux ou en visio. Artisans, gérants TPE, dirigeants PME et prestataires du Morbihan intérieur souhaitant maîtriser l'IA au quotidien.",
      en: "Coaching 1-to-1 à Guer : sessions dans vos locaux ou en visio. Artisans, gérants TPE, dirigeants PME et prestataires du Morbihan intérieur souhaitant maîtriser l'IA au quotidien.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Guer et dans le Morbihan intérieur ?",
      a: "Oui, Axion-IA intervient à Guer et dans le Morbihan intérieur (Ploërmel, Malestroit, Josselin, Mauron, Beignon). Couverture Vannes (45 min), Rennes (55 min) et axe Ploërmel-Redon.",
    },
    {
      q: "Travaillez-vous avec les prestataires du secteur défense à Guer ?",
      a: "Oui, nous accompagnons les prestataires et sous-traitants du secteur défense à Guer sur leurs cas IA : automatisation gestion documentaire, classification de données, agents IA support logistique, optimisation processus administratifs et chatbots internes.",
    },
    {
      q: "Accompagnez-vous les TPE et commerces de Guer ?",
      a: "Oui, nous accompagnons TPE artisanales et commerces de Guer sur des cas IA concrets : automatisation devis et facturation, chatbots relation client, gestion stocks, agents IA prise de RDV. Audit Flash 490 € HT adapté aux petites structures.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Guer ?",
      a: "L'audit Flash IA démarre à 490 € HT pour les TPE et PME de Guer et du Morbihan intérieur. Formats supérieurs disponibles selon la taille et la complexité de votre activité. Devis transparent, sans engagement.",
    },
    {
      q: "Comment se déroule une mission IA sur site à Guer ?",
      a: "Nos consultants se déplacent dans vos locaux à Guer ou interviennent en visio. Gare de Guer accessible en TER depuis Rennes et Vannes. Pour les missions multi-jours, hébergement local possible. Frais de déplacement transparents en sus du forfait.",
    },
  ],
};
