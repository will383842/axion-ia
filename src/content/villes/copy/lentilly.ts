// AUTO-GENERATED 2026-05-27 — T4 batch #12 Claude Code (Will).
// Lentilly (69, Rhône) — banlieue NO Lyon, péri-urbain résidentiel, monts du Lyonnais est.

import type { VilleCopy } from "./types";

export const LENTILLY_COPY: VilleCopy = {
  pitchFr:
    "Lentilly (69), commune en banlieue nord-ouest de Lyon dans le Rhône, est positionnée aux portes des monts du Lyonnais. Tissu péri-urbain résidentiel : PME tertiaires, TPE artisans BTP, commerces, professions libérales. Axion-IA accompagne tout le tissu professionnel local du NO lyonnais.",
  pitchEn:
    "Lentilly (69), commune en banlieue nord-ouest de Lyon dans le Rhône, est positionnée aux portes des monts du Lyonnais. Tissu péri-urbain résidentiel : PME tertiaires, TPE artisans BTP, commerces, professions libérales. Axion-IA accompagne tout le tissu professionnel local du NO lyonnais.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Lentilly (69). Nous accompagnons tous types d'activités : PME tertiaires NO lyonnais, TPE artisans BTP, commerçants, professions libérales, indépendants et services aux entreprises de la banlieue nord-ouest lyonnaise.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Lentilly (69). Nous accompagnons tous types d'activités : PME tertiaires NO lyonnais, TPE artisans BTP, commerçants, professions libérales, indépendants et services aux entreprises de la banlieue nord-ouest lyonnaise.",
  seoHook: "tertiaires lyonnais",
  ecosystemFr:
    "Lentilly est positionnée en banlieue NO de Lyon, aux portes des monts du Lyonnais est. Tissu péri-urbain résidentiel : PME tertiaires, professions libérales, TPE artisanat BTP, commerces de proximité.",
  ecosystemEn:
    "Lentilly est positionnée en banlieue NO de Lyon, aux portes des monts du Lyonnais est. Tissu péri-urbain résidentiel : PME tertiaires, professions libérales, TPE artisanat BTP, commerces de proximité.",
  distancesFr:
    "Lyon centre à 25 min. Aéroport Lyon-Saint-Exupéry à 50 min. L'Arbresle à 5 min. Dardilly à 15 min. Écully à 15 min.",
  distancesEn:
    "Lyon centre à 25 min. Aéroport Lyon-Saint-Exupéry à 50 min. L'Arbresle à 5 min. Dardilly à 15 min. Écully à 15 min.",
  topSectorsNaf: [
    "PME tertiaires NO lyonnais",
    "Professions libérales",
    "TPE artisanat BTP",
    "Commerce de proximité",
    "Services aux entreprises",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Lentilly : diagnostic adapté aux PME tertiaires NO lyonnais, professions libérales, TPE artisans. Audit Flash {{price:audit-flash|flat}} avec ROI quantifié.",
      en: "Audit IA à Lentilly : diagnostic adapté aux PME tertiaires NO lyonnais, professions libérales, TPE artisans. Audit Flash {{price:audit-flash|flat}} avec ROI quantifié.",
    },
    interventions: {
      fr: "Interventions IA sur site à Lentilly : formats adaptés au bureau, cabinet, commerce. Vos équipes repartent autonomes sur outils IA.",
      en: "Interventions IA sur site à Lentilly : formats adaptés au bureau, cabinet, commerce. Vos équipes repartent autonomes sur outils IA.",
    },
    implementation: {
      fr: "Implémentation IA à Lentilly : agents conversationnels B2B/patient, automatisation workflow, classification dossiers, CRM augmentés. NO lyonnais.",
      en: "Implémentation IA à Lentilly : agents conversationnels B2B/patient, automatisation workflow, classification dossiers, CRM augmentés. NO lyonnais.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Lentilly : sessions dans votre bureau, cabinet, commerce ou en visio. Dirigeants tertiaires, professions libérales, artisans, gérants TPE/PME lentillois.",
      en: "Coaching 1-to-1 à Lentilly : sessions dans votre bureau, cabinet, commerce ou en visio. Dirigeants tertiaires, professions libérales, artisans, gérants TPE/PME lentillois.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Lentilly et NO lyonnais ?",
      a: "Oui, Axion-IA intervient à Lentilly et tout le NO lyonnais (L'Arbresle à 5 min, Dardilly, Écully, La Tour-de-Salvagny). Consultants depuis Lyon (25 min).",
    },
    {
      q: "Accompagnez-vous les professions libérales lentillois ?",
      a: "Oui, nous accompagnons les professions libérales à Lentilly sur leurs cas IA respectueux du secret professionnel : agents conversationnels, automatisation RDV, classification dossiers. RGPD strict.",
    },
    {
      q: "Travaillez-vous avec les PME tertiaires ?",
      a: "Oui, nous accompagnons les PME tertiaires NO lyonnais sur leurs cas IA : agents conversationnels B2B, automatisation workflow, classification documents, CRM augmentés.",
    },
    {
      q: "Quels services pour les TPE artisans et commerces ?",
      a: "Pour les TPE artisans BTP et commerçants à Lentilly, audit Flash {{price:audit-flash|flat}}, intervention 1 journée formation, coaching 1-to-1.",
    },
    {
      q: "Comment se passe une mission depuis Lyon ?",
      a: "Nos consultants viennent de Lyon (25 min). Déplacements facturés séparément, grille tarifaire publique.",
    },
  ],
};
