// AUTO-GENERATED 2026-05-27 — T4 batch #21 Claude Code (Will).
// Auneau-Bleury-Saint-Symphorien (28, Eure-et-Loir) — Commune nouvelle, Beauce, 6 419 hab.

import type { VilleCopy } from "./types";

export const AUNEAU_BLEURY_SAINT_SYMPHORIEN_COPY: VilleCopy = {
  pitchFr:
    "Auneau-Bleury-Saint-Symphorien (28), commune nouvelle d'Eure-et-Loir au cœur de la Beauce, est un pôle rural dynamique entre Chartres et Étampes. Tissu TPE/PME agriculture céréalière Beauce, agroalimentaire, artisanat, commerces de bourg et services de proximité. Axion-IA y intervient pour tous types d'activités.",
  pitchEn:
    "Auneau-Bleury-Saint-Symphorien (28), commune nouvelle d'Eure-et-Loir au cœur de la Beauce, est un pôle rural dynamique entre Chartres et Étampes. Tissu TPE/PME agriculture céréalière Beauce, agroalimentaire, artisanat, commerces de bourg et services de proximité. Axion-IA y intervient pour tous types d'activités.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Auneau-Bleury-Saint-Symphorien (28). Nous accompagnons tous types d'activités : exploitants céréaliers de Beauce, TPE/PME agroalimentaire, artisans, commerçants de bourg, professions libérales et services aux entreprises rurales.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Auneau-Bleury-Saint-Symphorien (28). Nous accompagnons tous types d'activités : exploitants céréaliers de Beauce, TPE/PME agroalimentaire, artisans, commerçants de bourg, professions libérales et services aux entreprises rurales.",
  ecosystemFr:
    "Auneau-Bleury-Saint-Symphorien est une commune nouvelle née en 2016 de la fusion d'Auneau, Bleury et Saint-Symphorien, au cœur de la Beauce céréalière. L'économie locale repose sur l'agriculture grandes cultures (blé, colza, orge), l'agroalimentaire, l'artisanat BTP, les commerces de bourg et les services de proximité.",
  ecosystemEn:
    "Auneau-Bleury-Saint-Symphorien est une commune nouvelle née en 2016 de la fusion d'Auneau, Bleury et Saint-Symphorien, au cœur de la Beauce céréalière. L'économie locale repose sur l'agriculture grandes cultures (blé, colza, orge), l'agroalimentaire, l'artisanat BTP, les commerces de bourg et les services de proximité.",
  distancesFr:
    "Chartres à 25 min. Étampes à 25 min. Paris à 1h15 via A10/A11. Gare d'Auneau-Aunay (TER vers Chartres). Rambouillet à 30 min.",
  distancesEn:
    "Chartres à 25 min. Étampes à 25 min. Paris à 1h15 via A10/A11. Gare d'Auneau-Aunay (TER vers Chartres). Rambouillet à 30 min.",
  topSectorsNaf: [
    "Agriculture céréalière Beauce",
    "Agroalimentaire & négoce grains",
    "TPE artisanat BTP",
    "Commerce de bourg",
    "Services de proximité",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Auneau-Bleury-Saint-Symphorien : diagnostic adapté aux exploitants céréaliers Beauce, TPE/PME agroalimentaire, artisans, commerçants et professions libérales. ROI chiffré, audit Flash 490 € HT.",
      en: "Audit IA à Auneau-Bleury-Saint-Symphorien : diagnostic adapté aux exploitants céréaliers Beauce, TPE/PME agroalimentaire, artisans, commerçants et professions libérales. ROI chiffré, audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Auneau-Bleury-Saint-Symphorien : formats adaptés à l'exploitation céréalière, atelier agroalimentaire, commerce ou cabinet. Vos équipes repartent autonomes sur outils IA configurés métier.",
      en: "Interventions IA sur site à Auneau-Bleury-Saint-Symphorien : formats adaptés à l'exploitation céréalière, atelier agroalimentaire, commerce ou cabinet. Vos équipes repartent autonomes sur outils IA configurés métier.",
    },
    implementation: {
      fr: "Implémentation IA à Auneau-Bleury-Saint-Symphorien : agriculture de précision céréales Beauce, automatisation gestion exploitation, prédiction rendements, agents conversationnels B2B négoce grains.",
      en: "Implémentation IA à Auneau-Bleury-Saint-Symphorien : agriculture de précision céréales Beauce, automatisation gestion exploitation, prédiction rendements, agents conversationnels B2B négoce grains.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Auneau-Bleury-Saint-Symphorien : sessions dans votre exploitation, commerce, bureau ou en visio. Exploitants Beauce, artisans, commerçants, gérants TPE/PME rurales.",
      en: "Coaching 1-to-1 à Auneau-Bleury-Saint-Symphorien : sessions dans votre exploitation, commerce, bureau ou en visio. Exploitants Beauce, artisans, commerçants, gérants TPE/PME rurales.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Auneau-Bleury-Saint-Symphorien et en Beauce ?",
      a: "Oui, Axion-IA intervient à Auneau-Bleury-Saint-Symphorien et toute la Beauce eurélienne (Janville-en-Beauce, Voves, Orgères-en-Beauce). Consultants depuis Paris (1h15) ou Chartres (25 min).",
    },
    {
      q: "Travaillez-vous avec les exploitants céréaliers de Beauce ?",
      a: "Oui, nous accompagnons les exploitants céréaliers de Beauce sur leurs cas IA : agriculture de précision, prédiction rendements, automatisation gestion parcelles, optimisation intrants, vente directe.",
    },
    {
      q: "Quels tarifs pour un audit IA à Auneau-Bleury-Saint-Symphorien ?",
      a: "Audit Flash 490 € HT. Tarifs publics et transparents pour TPE/PME et exploitants agricoles. ROI chiffré dès le diagnostic initial.",
    },
    {
      q: "Accompagnez-vous les commerces et artisans de la commune nouvelle ?",
      a: "Oui, nos formats sont adaptés aux commerçants de bourg et artisans BTP : automatisation devis, chatbots prise de RDV, gestion stocks, agents IA plannings, présence en ligne.",
    },
    {
      q: "Comment se passe une mission à Auneau-Bleury-Saint-Symphorien ?",
      a: "Nos consultants se déplacent depuis Paris (1h15 via A10/A11) ou Chartres (25 min). Gare d'Auneau-Aunay (TER vers Chartres). Frais de déplacement facturés à part du forfait, tarifs publics.",
    },
  ],
};
