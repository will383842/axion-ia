// AUTO-GENERATED 2026-05-27 — T4 batch #25 Claude Code (Will).
// Beaufort-en-Anjou (49, Maine-et-Loire) — commune nouvelle, Anjou, liqueur Cointreau.

import type { VilleCopy } from "./types";

export const BEAUFORT_EN_ANJOU_COPY: VilleCopy = {
  pitchFr:
    "Beaufort-en-Anjou (49), commune nouvelle entre Baugeois et Val d'Authion, compte près de 6 900 habitants dans un territoire agricole et commerçant. Axion-IA y intervient pour tous types d'activités : agriculture, artisanat, commerces de bourg, PME et services de proximité.",
  pitchEn:
    "Beaufort-en-Anjou (49), commune nouvelle entre Baugeois et Val d'Authion, compte près de 6 900 habitants dans un territoire agricole et commerçant. Axion-IA y intervient pour tous types d'activités : agriculture, artisanat, commerces de bourg, PME et services de proximité.",
  directAnswerFr:
    "Axion-IA est un cabinet de conseil en intelligence artificielle qui intervient à Beaufort-en-Anjou (49). Nous accompagnons les TPE, PME et exploitants agricoles du Beaufortais dans l'adoption de l'IA : audit Flash dès 490 € HT, interventions sur site, implémentation d'agents IA et coaching dirigeants 1-to-1. RGPD strict, tarifs publics.",
  directAnswerEn:
    "Axion-IA est un cabinet de conseil en intelligence artificielle qui intervient à Beaufort-en-Anjou (49). Nous accompagnons les TPE, PME et exploitants agricoles du Beaufortais dans l'adoption de l'IA : audit Flash dès 490 € HT, interventions sur site, implémentation d'agents IA et coaching dirigeants 1-to-1. RGPD strict, tarifs publics.",
  seoHook: "TPE & PME Maine-et-Loire",
  ecosystemFr:
    "Beaufort-en-Anjou est un bourg-centre dynamique entre Baugeois et Val d'Authion. L'économie repose sur l'agriculture (grandes cultures, horticulture, semences végétales), l'artisanat, les commerces de centre-bourg et les PME agroalimentaires. Territoire historique de la liqueur et des vergers angevins.",
  ecosystemEn:
    "Beaufort-en-Anjou est un bourg-centre dynamique entre Baugeois et Val d'Authion. L'économie repose sur l'agriculture (grandes cultures, horticulture, semences végétales), l'artisanat, les commerces de centre-bourg et les PME agroalimentaires. Territoire historique de la liqueur et des vergers angevins.",
  distancesFr:
    "Angers à 25 min via D347. Baugé-en-Anjou à 15 min. Gare TGV Angers Saint-Laud à 30 min. Saumur à 45 min. Accès D347/D59. Pas de gare ferroviaire directe.",
  distancesEn:
    "Angers à 25 min via D347. Baugé-en-Anjou à 15 min. Gare TGV Angers Saint-Laud à 30 min. Saumur à 45 min. Accès D347/D59. Pas de gare ferroviaire directe.",
  topSectorsNaf: [
    "Agriculture & semences végétales",
    "Horticulture & pépinières",
    "Commerce de bourg & proximité",
    "Artisanat BTP & second œuvre",
    "Industrie agroalimentaire",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Beaufort-en-Anjou : diagnostic adapté aux exploitants agricoles, horticulteurs et TPE/PME du Beaufortais. Audit Flash 490 € HT, ROI chiffré, restitution sous 5 jours.",
      en: "Audit IA à Beaufort-en-Anjou : diagnostic adapté aux exploitants agricoles, horticulteurs et TPE/PME du Beaufortais. Audit Flash 490 € HT, ROI chiffré, restitution sous 5 jours.",
    },
    interventions: {
      fr: "Interventions IA sur site à Beaufort-en-Anjou : formations pour équipes agricoles, horticoles et commerciales. Ateliers pratiques adaptés au contexte Beaufortais.",
      en: "Interventions IA sur site à Beaufort-en-Anjou : formations pour équipes agricoles, horticoles et commerciales. Ateliers pratiques adaptés au contexte Beaufortais.",
    },
    implementation: {
      fr: "Implémentation IA à Beaufort-en-Anjou : automatisation traçabilité semences, gestion parcellaire, devis artisans, CRM commerces de bourg. Solutions adaptées agriculture et horticulture.",
      en: "Implémentation IA à Beaufort-en-Anjou : automatisation traçabilité semences, gestion parcellaire, devis artisans, CRM commerces de bourg. Solutions adaptées agriculture et horticulture.",
    },
    unAUn: {
      fr: "Coaching IA 1-to-1 à Beaufort-en-Anjou : sessions pour dirigeants TPE/PME, exploitants agricoles, horticulteurs et commerçants du Beaufortais.",
      en: "Coaching IA 1-to-1 à Beaufort-en-Anjou : sessions pour dirigeants TPE/PME, exploitants agricoles, horticulteurs et commerçants du Beaufortais.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Beaufort-en-Anjou ?",
      a: "Oui, Axion-IA intervient à Beaufort-en-Anjou et dans le Beaufortais (Baugé-en-Anjou, Longué-Jumelles, Mazé-Milon). Nos consultants se déplacent sur site, démarrage sous 5 jours.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Beaufort-en-Anjou ?",
      a: "L'Audit Flash IA à Beaufort-en-Anjou démarre à 490 € HT pour une session de 4 heures. Diagnostic complet, ROI chiffré et plan d'action concret inclus. Tarifs publics.",
    },
    {
      q: "Accompagnez-vous les horticulteurs du Beaufortais ?",
      a: "Oui, nous accompagnons les horticulteurs à Beaufort-en-Anjou : IA pour gestion stocks végétaux, automatisation commandes pépinière, traçabilité semences, prédiction météo culture, CRM clients professionnels.",
    },
    {
      q: "Quels secteurs ciblez-vous à Beaufort-en-Anjou ?",
      a: "Axion-IA accompagne tous types d'activités à Beaufort-en-Anjou : agriculture, horticulture, commerce de bourg, artisanat BTP et agroalimentaire. Pas de restriction sectorielle.",
    },
    {
      q: "Comment se déroule une mission dans le Beaufortais ?",
      a: "Nos consultants seniors se déplacent dans vos locaux à Beaufort-en-Anjou. Diagnostic initial, ateliers pratiques, livraison d'outils IA configurés. Frais déplacement depuis Angers (25 min) en transparence.",
    },
  ],
};
