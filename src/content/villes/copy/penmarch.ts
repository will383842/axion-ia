// AUTO-GENERATED 2026-05-27 — T4 batch #21 Claude Code (Will).
// Penmarch (29, Finistère) — Pays Bigouden, port Saint-Guénolé, pêche et phare d'Eckmühl.

import type { VilleCopy } from "./types";

export const PENMARCH_COPY: VilleCopy = {
  pitchFr:
    "Penmarch (29), commune emblématique du Pays Bigouden en Finistère sud, est connue pour son port de pêche de Saint-Guénolé, son phare d'Eckmühl et son littoral préservé. TPE et PME maritimes, mareyeurs, conserveries, artisans du bâtiment, commerces de bourg, hébergeurs et restaurateurs touristiques. Axion-IA y intervient pour tous types d'activités.",
  pitchEn:
    "Penmarch (29), commune emblématique du Pays Bigouden en Finistère sud, est connue pour son port de pêche de Saint-Guénolé, son phare d'Eckmühl et son littoral préservé. TPE et PME maritimes, mareyeurs, conserveries, artisans du bâtiment, commerces de bourg, hébergeurs et restaurateurs touristiques. Axion-IA y intervient pour tous types d'activités.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Penmarch (29). Nous accompagnons TPE et PME : pêcheurs et mareyeurs du port de Saint-Guénolé, conserveries, artisans du bâtiment, commerces de bourg, hébergeurs et restaurateurs touristiques. Audit Flash dès 490 € HT.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Penmarch (29). Nous accompagnons TPE et PME : pêcheurs et mareyeurs du port de Saint-Guénolé, conserveries, artisans du bâtiment, commerces de bourg, hébergeurs et restaurateurs touristiques. Audit Flash dès 490 € HT.",
  seoHook: "pêche mareyage & conserveries agroalimentaire",
  ecosystemFr:
    "Penmarch est une commune du Pays Bigouden, en Finistère sud, célèbre pour son port de pêche de Saint-Guénolé (3e port français en tonnage débarqué historique) et le phare d'Eckmühl. Tissu TPE/PME : pêche et mareyage, conserveries, artisanat du bâtiment, commerces de bourg, hébergement et restauration touristique.",
  ecosystemEn:
    "Penmarch est une commune du Pays Bigouden, en Finistère sud, célèbre pour son port de pêche de Saint-Guénolé (3e port français en tonnage débarqué historique) et le phare d'Eckmühl. Tissu TPE/PME : pêche et mareyage, conserveries, artisanat du bâtiment, commerces de bourg, hébergement et restauration touristique.",
  distancesFr:
    "Quimper à 30 min en voiture. Gare TGV Quimper accessible en 35 min. Aéroport Quimper-Cornouaille à 25 min. Plage de la Torche à 10 min. Pont-l'Abbé (sous-préfecture) à 15 min.",
  distancesEn:
    "Quimper à 30 min en voiture. Gare TGV Quimper accessible en 35 min. Aéroport Quimper-Cornouaille à 25 min. Plage de la Torche à 10 min. Pont-l'Abbé (sous-préfecture) à 15 min.",
  topSectorsNaf: [
    "Pêche et mareyage",
    "Conserveries et agroalimentaire mer",
    "Artisanat du bâtiment",
    "Commerce de bourg et services",
    "Hébergement et restauration touristique",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Penmarch : diagnostic adapté aux TPE pêche et mareyage du port Saint-Guénolé, conserveries, artisans, commerces de bourg, hébergeurs touristiques. ROI chiffré, Audit Flash 490 € HT.",
      en: "Audit IA à Penmarch : diagnostic adapté aux TPE pêche et mareyage du port Saint-Guénolé, conserveries, artisans, commerces de bourg, hébergeurs touristiques. ROI chiffré, Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Penmarch : formats adaptés criée et atelier mareyage, ligne conserverie, chantier artisan, commerce ou restaurant. Équipes formées à l'autonomie sur les outils IA.",
      en: "Interventions IA sur site à Penmarch : formats adaptés criée et atelier mareyage, ligne conserverie, chantier artisan, commerce ou restaurant. Équipes formées à l'autonomie sur les outils IA.",
    },
    implementation: {
      fr: "Implémentation IA à Penmarch : prévision marées et météo pour pêche, traçabilité produits mer, optimisation lignes conserveries, devis automatisés artisans, réservations hébergeurs touristiques.",
      en: "Implémentation IA à Penmarch : prévision marées et météo pour pêche, traçabilité produits mer, optimisation lignes conserveries, devis automatisés artisans, réservations hébergeurs touristiques.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Penmarch : sessions au port Saint-Guénolé, en conserverie, en boutique ou en visio. Patrons pêcheurs, mareyeurs, dirigeants conserveries, artisans, commerçants et hébergeurs.",
      en: "Coaching 1-to-1 à Penmarch : sessions au port Saint-Guénolé, en conserverie, en boutique ou en visio. Patrons pêcheurs, mareyeurs, dirigeants conserveries, artisans, commerçants et hébergeurs.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Penmarch et dans le Pays Bigouden ?",
      a: "Oui, Axion-IA intervient à Penmarch et dans tout le Pays Bigouden (Le Guilvinec, Pont-l'Abbé, Plomeur, Loctudy, Plobannalec-Lesconil). Quimper à 30 min en voiture.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Penmarch ?",
      a: "L'Audit Flash démarre à 490 € HT. Tarifs publics consultables en ligne, adaptés aux TPE pêche, mareyeurs, conserveries et artisans du Pays Bigouden.",
    },
    {
      q: "Travaillez-vous avec les pêcheurs et mareyeurs de Saint-Guénolé ?",
      a: "Oui, prévision marées et météo, optimisation tournées de pêche, traçabilité produits mer, gestion commandes professionnels, conformité sanitaire et étiquetage pour pêcheurs et mareyeurs.",
    },
    {
      q: "Accompagnez-vous les conserveries et artisans du Pays Bigouden ?",
      a: "Oui, optimisation lignes de production conserveries, traçabilité matière première mer, devis automatisés artisans, planning chantiers, suivi clients et fidélisation locale.",
    },
    {
      q: "Comment accéder à Penmarch depuis Quimper ?",
      a: "Penmarch est à 30 min de Quimper en voiture (D785). Gare TGV Quimper à 35 min, aéroport Quimper-Cornouaille à 25 min. Frais de déplacement facturés à part.",
    },
  ],
};
