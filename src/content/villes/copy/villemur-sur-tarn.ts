// AUTO-GENERATED 2026-05-27 — T4 batch #24 Claude Code (Will).
// Villemur-sur-Tarn (31, Haute-Garonne) — Bords Tarn, nord Toulouse.

import type { VilleCopy } from "./types";

export const VILLEMUR_SUR_TARN_COPY: VilleCopy = {
  pitchFr:
    "Villemur-sur-Tarn (31), commune des bords du Tarn au nord de Toulouse, plus de 6 300 habitants. Tissu TPE artisans, PME industrielles et agroalimentaires, commerces de centre-ville, professions libérales. Axion-IA y intervient pour tous types d'activités du nord de la Haute-Garonne.",
  pitchEn:
    "Villemur-sur-Tarn (31), commune des bords du Tarn au nord de Toulouse, plus de 6 300 habitants. Tissu TPE artisans, PME industrielles et agroalimentaires, commerces de centre-ville, professions libérales. Axion-IA y intervient pour tous types d'activités du nord de la Haute-Garonne.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Villemur-sur-Tarn (31). Nous accompagnons TPE artisans, PME industrielles, commerçants et professions libérales du nord Haute-Garonne. Audit Flash dès 490 € HT.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Villemur-sur-Tarn (31). Nous accompagnons TPE artisans, PME industrielles, commerçants et professions libérales du nord Haute-Garonne. Audit Flash dès 490 € HT.",
  ecosystemFr:
    "Villemur-sur-Tarn est un bourg historique des bords du Tarn au nord de Toulouse. PME industrielles, artisans BTP, commerces de centre-ville, professions libérales, quelques exploitations agricoles. Ville-passerelle entre Toulouse nord et le Tarn-et-Garonne.",
  ecosystemEn:
    "Villemur-sur-Tarn est un bourg historique des bords du Tarn au nord de Toulouse. PME industrielles, artisans BTP, commerces de centre-ville, professions libérales, quelques exploitations agricoles. Ville-passerelle entre Toulouse nord et le Tarn-et-Garonne.",
  distancesFr:
    "Toulouse centre à 35 min (D630). Gare Matabiau à 40 min. Montauban à 30 min. Fronton à 15 min. Bruguières à 20 min.",
  distancesEn:
    "Toulouse centre à 35 min (D630). Gare Matabiau à 40 min. Montauban à 30 min. Fronton à 15 min. Bruguières à 20 min.",
  topSectorsNaf: [
    "Industrie & fabrication",
    "Artisanat & BTP",
    "Commerce de centre-ville",
    "Professions libérales & santé",
    "Agriculture & agroalimentaire",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Villemur-sur-Tarn : diagnostic adapté aux PME industrielles, TPE artisans et commerçants du nord Haute-Garonne. Audit Flash 490 € HT.",
      en: "Audit IA à Villemur-sur-Tarn : diagnostic adapté aux PME industrielles, TPE artisans et commerçants du nord Haute-Garonne. Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Villemur-sur-Tarn : formats adaptés à l'atelier industriel, au commerce, à l'artisan. Vos collaborateurs gèrent ensuite la stack en autonomie.",
      en: "Interventions IA sur site à Villemur-sur-Tarn : formats adaptés à l'atelier industriel, au commerce, à l'artisan. Vos collaborateurs gèrent ensuite la stack en autonomie.",
    },
    implementation: {
      fr: "Implémentation IA à Villemur-sur-Tarn : automatisation contrôle qualité, gestion stocks, classification documents, agents conversationnels pour PME du nord 31.",
      en: "Implémentation IA à Villemur-sur-Tarn : automatisation contrôle qualité, gestion stocks, classification documents, agents conversationnels pour PME du nord 31.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Villemur-sur-Tarn : sessions dirigeants PME industrielles, TPE artisans, commerçants du nord Haute-Garonne.",
      en: "Coaching 1-to-1 à Villemur-sur-Tarn : sessions dirigeants PME industrielles, TPE artisans, commerçants du nord Haute-Garonne.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Villemur-sur-Tarn ?",
      a: "Oui, Axion-IA intervient à Villemur-sur-Tarn et dans tout le nord de la Haute-Garonne (Fronton, Bruguières, Montauban). Nos consultants se déplacent en 35 min depuis Toulouse.",
    },
    {
      q: "Quels secteurs accompagnez-vous à Villemur-sur-Tarn ?",
      a: "Nous accompagnons tous types d'activités : PME industrielles, artisans, commerces, professions libérales, agriculture. Aucun secteur exclu.",
    },
    {
      q: "Quel est le tarif d'un audit IA à Villemur-sur-Tarn ?",
      a: "L'Audit Flash démarre à 490 € HT pour une TPE. Pour les PME industrielles, audit approfondi entre 1 900 et 3 900 € HT. Tarifs publics.",
    },
    {
      q: "Comment se déroule une intervention IA à Villemur-sur-Tarn ?",
      a: "Nos consultants se déplacent à Villemur-sur-Tarn (35 min depuis Toulouse). Intervention 1 journée minimum, atelier pratique dans vos locaux.",
    },
    {
      q: "L'IA peut-elle aider une PME industrielle à Villemur-sur-Tarn ?",
      a: "Oui, l'IA optimise contrôle qualité, planification production, gestion stocks, maintenance prédictive et classification documents techniques.",
    },
  ],
};
