// AUTO-GENERATED 2026-05-27 — T4 batch #23 Claude Code (Will).
// Aytre (17, Charente-Maritime) — Agglo La Rochelle sud, industrie ferroviaire.

import type { VilleCopy } from "./types";

export const AYTRE_COPY: VilleCopy = {
  pitchFr:
    "Aytré (17), commune au sud de l'agglomération de La Rochelle en Charente-Maritime, pôle industriel ferroviaire (site Alstom). Tissu de TPE artisans, PME industrielles et sous-traitants ferroviaires, commerces de proximité, services et professions libérales. Axion-IA y intervient pour tous types d'activités de l'agglomération rochelaise.",
  pitchEn:
    "Aytré (17), commune au sud de l'agglomération de La Rochelle en Charente-Maritime, pôle industriel ferroviaire (site Alstom). Tissu de TPE artisans, PME industrielles et sous-traitants ferroviaires, commerces de proximité, services et professions libérales. Axion-IA y intervient pour tous types d'activités de l'agglomération rochelaise.",
  directAnswerFr:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Aytré (17). Nous accompagnons tous types d'activités : TPE artisans, PME industrielles et sous-traitants ferroviaires, commerçants, professions libérales et indépendants de cette commune industrielle de l'agglomération de La Rochelle.",
  directAnswerEn:
    "Axion-IA est un cabinet d'architectes IA seniors qui intervient à Aytré (17). Nous accompagnons tous types d'activités : TPE artisans, PME industrielles et sous-traitants ferroviaires, commerçants, professions libérales et indépendants de cette commune industrielle de l'agglomération de La Rochelle.",
  ecosystemFr:
    "Aytré est une commune industrielle de l'agglomération de La Rochelle, connue pour son site Alstom (construction de TGV et trains régionaux). PME industrielles et sous-traitants ferroviaires, TPE artisans BTP, commerces de proximité, professions libérales. Accès plage (Plage d'Aytré-Plage du Platin) et littoral.",
  ecosystemEn:
    "Aytré est une commune industrielle de l'agglomération de La Rochelle, connue pour son site Alstom (construction de TGV et trains régionaux). PME industrielles et sous-traitants ferroviaires, TPE artisans BTP, commerces de proximité, professions libérales. Accès plage (Plage d'Aytré-Plage du Platin) et littoral.",
  distancesFr:
    "Aytré est à 5 min de La Rochelle centre (sud). Gare TGV de La Rochelle à 10 min (Paris Montparnasse 2h50, Bordeaux 2h). Aéroport La Rochelle-Île de Ré à 20 min. Axe N137/A10.",
  distancesEn:
    "Aytré est à 5 min de La Rochelle centre (sud). Gare TGV de La Rochelle à 10 min (Paris Montparnasse 2h50, Bordeaux 2h). Aéroport La Rochelle-Île de Ré à 20 min. Axe N137/A10.",
  topSectorsNaf: [
    "Industrie ferroviaire & sous-traitance Alstom",
    "PME industrielles",
    "TPE artisanat BTP",
    "Commerce de proximité",
    "Professions libérales & services",
  ],
  servicesContext: {
    audit: {
      fr: "Audit IA à Aytré : diagnostic adapté aux PME industrielles ferroviaires, sous-traitants, TPE artisans et commerces de l'agglomération de La Rochelle sud. Audit Flash 490 € HT.",
      en: "Audit IA à Aytré : diagnostic adapté aux PME industrielles ferroviaires, sous-traitants, TPE artisans et commerces de l'agglomération de La Rochelle sud. Audit Flash 490 € HT.",
    },
    interventions: {
      fr: "Interventions IA sur site à Aytré : formats adaptés à l'atelier industriel, au bureau d'études, au commerce ou au cabinet. Vos équipes repartent autonomes sur outils IA configurés métier.",
      en: "Interventions IA sur site à Aytré : formats adaptés à l'atelier industriel, au bureau d'études, au commerce ou au cabinet. Vos équipes repartent autonomes sur outils IA configurés métier.",
    },
    implementation: {
      fr: "Implémentation IA à Aytré : maintenance prédictive équipements ferroviaires, contrôle qualité pièces industrielles, traçabilité production, automatisation documentation technique et reporting.",
      en: "Implémentation IA à Aytré : maintenance prédictive équipements ferroviaires, contrôle qualité pièces industrielles, traçabilité production, automatisation documentation technique et reporting.",
    },
    unAUn: {
      fr: "Coaching 1-to-1 à Aytré : sessions dans vos locaux industriels, bureau ou en visio. Dirigeants TPE/PME industrielles, sous-traitants ferroviaires et artisans de l'agglomération rochelaise souhaitant intégrer l'IA.",
      en: "Coaching 1-to-1 à Aytré : sessions dans vos locaux industriels, bureau ou en visio. Dirigeants TPE/PME industrielles, sous-traitants ferroviaires et artisans de l'agglomération rochelaise souhaitant intégrer l'IA.",
    },
  },
  faqGeolocalisee: [
    {
      q: "Axion-IA intervient-il à Aytré et dans l'agglomération de La Rochelle ?",
      a: "Oui, Axion-IA intervient à Aytré et toute l'agglomération de La Rochelle (Périgny, Lagord, Dompierre-sur-Mer, Nieul-sur-Mer). Nos consultants accèdent facilement via la gare TGV de La Rochelle à 10 min.",
    },
    {
      q: "Quels services IA pour les entreprises ferroviaires à Aytré ?",
      a: "Nous accompagnons les PME industrielles et sous-traitants ferroviaires d'Aytré : maintenance prédictive matériel roulant, contrôle qualité pièces, traçabilité production, automatisation documentation technique et optimisation processus fabrication.",
    },
    {
      q: "Accompagnez-vous les TPE et PME à Aytré ?",
      a: "Oui, nous accompagnons les TPE artisans, PME industrielles, commerçants et professions libérales d'Aytré : automatisation gestion administrative, agents IA relation client, optimisation processus et outils IA productivité.",
    },
    {
      q: "Quel est le tarif pour un audit IA à Aytré ?",
      a: "L'audit Flash IA à Aytré démarre à 490 € HT. Diagnostic complet de vos processus industriels, ferroviaires ou tertiaires, identification des cas d'usage IA prioritaires, ROI chiffré et plan d'action concret.",
    },
    {
      q: "Comment se déroule une mission IA sur site à Aytré ?",
      a: "Nos consultants accèdent à Aytré via la gare TGV de La Rochelle (10 min, Paris 2h50 direct). Mission sur site dans vos locaux industriels, bureau ou atelier. Frais de déplacement facturés à part du forfait mission.",
    },
  ],
};
