...
export const AUDIT_BY_SIZE: ReadonlyArray<AuditSizeOption> = [
  ...
  },
  {
    ...
    taglineFr: `PME en croissance, plusieurs services concernés. Audit ciblé pour un département précis (${getTierById(AUDIT_TIERS, "audit-cible").priceMin!}-${getTierById(AUDIT_TIERS, "audit-cible").priceMax!} €) ou audit stratégique multi-services (${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMin!}-${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMax!} €) selon votre ambition.`,
    taglineEn: `Growing SME, several departments concerned. Targeted audit on one department (€${getTierById(AUDIT_TIERS, "audit-cible").priceMin!}-${getTierById(AUDIT_TIERS, "audit-cible").priceMax!}) or strategic multi-department audit (€${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMin!}-${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMax!}) depending on ambition.`,
...
export const AUDIT_BY_SITUATION: ReadonlyArray<AuditSituationOption> = [
  ...
  },
  {
    ...
    taglineFr: `Vous avez déjà testé l'IA, vous voulez maintenant le grand jeu : audit stratégique multi-services PME (${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMin!}-${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMax!} €). Cartographie tous départements, plan d'exécution chiffré, roadmap 12-24 mois.`,
    taglineEn: `You've tested AI, now you want the full picture: strategic multi-department SME audit (€${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMin!}-${getTierById(AUDIT_TIERS, "audit-strategique-pme").priceMax!}). All-department mapping, quantified execution plan, 12-24 month roadmap.`,
    tier: "audit-strategique-pme",
    pathFr: "/audit/strategique-pme",
    pathEn: "/audit/strategic-pme",
    iconName: "Workflow",
...
export const AUDIT_TIERS_META: ReadonlyArray<AuditTierMeta> = [
  ...
  },
  {
    ...
    taglineFr: `Audit focalisé sur 1 département (marketing, RH, ops, finance, juridique). 3 sous-tiers : Solo / Standard / Avancé (${getTierById(AUDIT_TIERS, "audit-cible").priceMin!} → ${getTierById(AUDIT_TIERS, "audit-cible").priceMax!} €).`,
    taglineEn: `Audit focused on 1 department (marketing, HR, ops, finance, legal). 3 sub-tiers: Solo / Standard / Advanced (€${getTierById(AUDIT_TIERS, "audit-cible").priceMin!} → €${getTierById(AUDIT_TIERS, "audit-cible").priceMax!}).`,
    audienceSizes: ["pme"],
    accent: "terracotta",
