/**
 * CONTENU RÉDIGÉ — « IA pour le marketing » (1 jour, 7 h, 2 modules au catalogue).
 *
 * ## Le fil rouge
 *
 ...
 * un message décliné sans réécriture, douze semaines planifiées avec un nom par
 * ligne. L'après-midi encaisse ce que la matinée a produit : commenter des
 * résultats sans exposer ses données, voir comment sa marque est reprise par un
 * assistant, et ne plus rien diffuser sans savoir ce qui a été vérifié.
 *
 * ## Le découpage, tel qu'il est au catalogue
 *
 * ⚠️ Le catalogue de cette formation range ses quatre temps pédagogiques dans
 * DEUX sections — « Matin » et « Après-midi ». L'import construit un module par
 * SECTION : « mod-1 » est donc la matinée entière (les temps intitulés
 * « Module 1 » et « Module 2 » dans les titres de séquences), « mod-2 »
 * l'après-midi entière. Les blocs écrits ici couvrent donc chacun plusieurs
 * séquences de même nature — d'où les deux démonstrations réunies dans un même
 * bloc, et les cinq ateliers réunis dans une même consigne.
 *
...
export const IA_POUR_LE_MARKETING: EnrichissementFormation = [
  // ───────────────────────────────────────────────────────────────────────────
  // mod-1 — Matin : le cadre, la voix de la marque, et le trimestre planifié
  // ───────────────────────────────────────────────────────────────────────────
  {
    moduleId: "mod-1",
    objectif: {
      enonce:
        "À la fin de cette matinée, vous produisez un contenu qui tient la voix de votre marque à partir d'un dossier de marque écrit, et vous déclinez un message unique sur plusieurs formats sans le réécrire.",
      objectifGlobalId: "obj-1",
      // La deuxième moitie de la matinee est entierement consacree a la
      // declinaison d'un message unique sur plusieurs formats.
      objectifsSecondairesIds: ["obj-2"],
      dureeMin: 15,
      notes: {
        script:
          "Tour de table en UNE phrase, pas une par personne qui s'étale : « votre prénom, et ce que vous devez produire chaque semaine ». Notez chaque réponse au tableau, elle servira de matière au calendrier éditorial de 11 h 30. Posez la règle du jour et laissez-la affichée : l'IA fait le premier jet, la marque reste la vôtre. Annoncez le livrable tout de suite — un dossier de marque écrit et douze semaines planifiées, en main ce soir. À la reprise après la pause, ne refaites pas d'introduction : une phrase suffit, « on passe d'un message à une série, et on ne repart plus de zéro chaque lundi ».",
        faq: [
          {
    ...
    demonstration: {
      ...
      notes: {
        ...
        planB:
          "Quota atteint ou service indisponible : les quatre sorties sont imprimées dans le kit formateur (pages 6 à 9, datées), avec le surlignage déjà appliqué. La comparaison à voix haute et le relevé de ce qui se dégrade se tiennent à l'identique sur papier. N'improvisez aucune sortie en direct : celles du kit ont été vérifiées.",
      },
    },
    pratique: {
      consigne:
        "Trois temps chronométrés. Premier temps : constituez votre dossier de marque sur la trame fournie — trois adjectifs de ton et trois contre-exemples (« jamais ceci »), vos cibles et personas, vos interdits de langage, trois contenus déjà validés qui servent d'étalon — puis testez-le immédiatement en relançant une publication réelle de votre entreprise avec ce dossier en contexte. Deuxième temps : déclinez un message réel de votre entreprise sur trois formats, en repartant de votre dossier de marque et non d'une consigne nue. Troisième temps : construisez le calendrier éditorial du trimestre à partir du tableau de sujets fourni — douze semaines, un sujet et un format par semaine, et le NOM de la personne qui tient chaque ligne.",
      aEmporter:
        "Le dossier de marque écrit et testé, un message réel décliné sur trois formats, et le calendrier des douze semaines avec un nom par ligne — les trois premières pièces du document unique remis en fin de journée.",
      dureeMin: 85,
      notes: {
        script:
          "Sur le dossier de marque : les trois CONTRE-exemples sont la partie utile, et c'est celle qu'on saute. Passez dans les rangs et exigez-les — « donnez-moi une phrase que votre entreprise n'écrirait jamais ». Tant que cette phrase n'est pas écrite, le dossier ne sert à rien. Sur la déclinaison : personne ne réécrit un format à la main, on relance avec le dossier en contexte, sinon l'atelier n'enseigne rien. Sur le calendrier : la colonne « qui » se remplit avec un PRÉNOM, pas avec « le marketing ». Annoncez le chronomètre à voix haute toutes les cinq minutes, et ne rédigez à la place de personne.",
        faq: [
          {
        ...
        planB:
          "Réseau tombé : le dossier de marque et le calendrier se remplissent intégralement à la main sur les trames papier — ils ne dépendent d'aucun outil. Pour la déclinaison, utilisez les quatre formats imprimés du kit (pages 6 à 9) et faites relever à la main ce que chaque format a retiré. Ce qui compte est la structure de la demande, pas l'outil qui l'exécute.",
      },
    },
    verification: {
      question:
        "Deux contrôles croisés en binôme, sur les grilles fournies. Sur la publication : tient-elle les trois adjectifs de ton déclarés au dossier de marque, la promesse qu'elle avance est-elle vérifiable dans un document, la mention de contenu généré est-elle due ? Puis relancez le MÊME brief avec le persona de votre voisin et relevez qui a disparu du texte. Sur la série et le calendrier : le dossier de marque est-il respecté sur les trois formats, l'appel à l'action est-il présent, et les douze semaines sont-elles tenables au vu des effectifs réellement annoncés ?",
      reponseAttendue:
        "Chaque écart est relevé et annoté au stylo sur la copie du binôme voisin. Le corrigé projeté liste les quatre dérives les plus fréquentes : le superlatif réintroduit, la promesse chiffrée non sourcée, l'appel à l'action absent du format court, et le calendrier dont trois lignes n'ont pas de titulaire.",
      dureeMin: 20,
      notes: {
        script:
          "Contrôle CROISÉ, jamais auto-correction : chaque binôme relit celui d'à côté. Le changement de persona est le temps fort — faites-le vraiment relancer, et faites dire à voix haute qui a disparu du texte. C'est le biais du BRIEF, avant celui de l'outil, et c'est la seule façon de le rendre visible. Projetez le corrigé après, jamais avant, et ne commentez que deux ou trois écarts, ceux qui se répètent dans la salle.",
        faq: [
          {
        ...
        planB:
          "Les grilles et le corrigé sont imprimés dans le kit (pages 16 et 17). Sans vidéoprojecteur, le corrigé se distribue et se commente à l'oral. Le changement de persona se fait alors sur les deux sorties pré-produites du kit, page 18.",
      },
    },
    synthese: {
      acquis: [
        "Je sais où je dépose un brief, et ce que je ne dépose jamais dans un outil grand public.",
        "J'ai un dossier de marque écrit, et je le remets en contexte avant chaque demande.",
        "Je décline un message sur trois formats sans le réécrire, et je planifie mon trimestre en une séance.",
      ],
      dureeMin: 10,
      notes: {
        script:
          "Faites FORMULER, ne récitez pas : « en une phrase, qu'est-ce que vous ne referez plus à partir de lundi ? ». Trois réponses suffisent. Faites nommer le fichier à voix haute — « dossier-de-marque-<entreprise>-<date> » : le dossier existe à partir du moment où il porte un nom. Annoncez l'après-midi en une phrase, sans promesse excessive : « on regarde ce que l'image sait faire et ce qu'elle ne sait pas, puis on commente des résultats sans exposer vos données ».",
        faq: [
          {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // mod-2 — Après-midi : image, résultats, visibilité, et ce qu'on diffuse
  // ───────────────────────────────────────────────────────────────────────────
  {
    moduleId: "mod-2",
    objectif: {
      enonce:
        "À la fin de cet après-midi, vous commentez les résultats d'une campagne sans exposer vos données, et vous ne diffusez plus un contenu sans savoir ce qui a été vérifié et par qui.",
      objectifGlobalId: "obj-3",
      // La veille concurrentielle et la visibilite de marque occupent un atelier
      // entier ; la chasse a l'erreur et la grille de relecture en occupent deux.
      objectifsSecondairesIds: ["obj-4", "obj-5"],
      dureeMin: 10,
      notes: {
        script:
          "Ouvrez sur le périmètre, pas sur la promesse : « trois usages très attendus cet après-midi, et trois périmètres honnêtes ». Annoncez tout de suite ce que l'IA ne fera pas — elle ne produira pas vos visuels de marque et elle ne calculera pas vos chiffres. Une salle prévenue en début d'après-midi ne se sent pas flouée à 17 h. À la reprise après la pause, une phrase suffit : « on termine par ce qui vous protège — ne plus rien diffuser sans savoir ce qui a été vérifié, et par qui ».",
        faq: [
          {
    ...
    demonstration: {
      avant:
        "On demande à l'outil de commenter une campagne en lui collant l'export complet — donc le fichier de contacts avec — et on le laisse recalculer les totaux. Sur le marché, on lui demande un paragraphe de tendances et on obtient des chiffres précis, sans aucune source, que personne ne saura vérifier.",
      apres:
        "On décrit la STRUCTURE du tableau sans jamais transmettre l'export, on saisit à la main les quatre valeurs à commenter, et on interdit tout calcul. Sur le marché, on exige une source nommée pour chaque affirmation chiffrée — et le paragraphe se vide de ce qui n'était pas vérifiable.",
      prompt:
        "DÉMONSTRATION A — commenter des résultats de campagne sans déposer l'export.\n\n« Je te décris la structure de mon tableau de campagne, je ne te le transmets pas. Colonnes : semaine, nombre d'envois, taux d'ouverture, taux de clic, nombre de demandes de devis. Voici les quatre valeurs que je saisis moi-même, semaine 36 : taux d'ouverture 31 %, taux de clic 2,4 %, 7 demandes de devis, contre 4 la semaine précédente. Rédige en 120 mots le commentaire de ces résultats pour un comité de direction : ce qui progresse, ce qui stagne, et la question que ces chiffres ne permettent pas de trancher. Ne calcule rien, ne recalcule aucun pourcentage, n'ajoute aucun chiffre que je ne t'ai pas donné. »\n\nDÉMONSTRATION B — le même paragraphe sans contrainte de source, puis avec.\n\nSans contrainte : « Rédige un paragraphe de 120 mots sur les tendances du marché de la menuiserie professionnelle en France. »\n\nAvec contrainte : « Rédige un paragraphe de 120 mots sur les tendances du marché de la menuiserie professionnelle en France. Pour CHAQUE affirmation chiffrée, indique entre crochets la source précise et son année. Si tu ne disposes pas d'une source que tu peux nommer exactement, n'écris pas l'affirmation : écris à sa place [AUCUNE SOURCE DISPONIBLE]. Ne cite aucune étude dont tu n'es pas certain du titre. »",
      outil: "Un seul outil, celui validé dans la salle (Claude ou ChatGPT selon le groupe).",
      captureEcran:
        "Trois visuels du kit annotés — ce qui tient, ce qui rate — puis les deux commentaires de campagne côte à côte avec le total recalculé par l'outil entouré en rouge, et enfin le paragraphe de marché avant / après, les [AUCUNE SOURCE DISPONIBLE] surlignés.",
      verifieLe: VERIFIE_LE,
      dureeMin: 25,
      notes: {
        script:
          "Sur les visuels : commentez les quatre images du kit sans ouvrir aucun compte, et nommez les deux colonnes à voix haute — ce qui tient (illustration d'ambiance, déclinaison de gabarit, recadrage, texte alternatif), ce qui rate systématiquement (le texte dans l'image, la charte, le logo, le visuel de marque). Ne survendez rien, la salle vous le rendra. Sur les résultats : demandez d'abord un TOTAL à l'outil et laissez-le se tromper devant tout le monde — c'est la démonstration la plus rentable de l'après-midi. Puis reposez la demande sans calcul. Sur les sources : lisez la première version à voix haute avec assurance, la salle doit sentir à quel point c'est convaincant, puis affichez la seconde. Le contraste se suffit, ne le commentez pas trop.",
        faq: [
          {
        ...
        planB:
          "Les quatre visuels sont déjà imprimés dans le kit, la revue ne dépend d'aucun outil. Les deux commentaires de campagne et les deux paragraphes de marché sont imprimés pages 22 à 25, datés, avec le chiffre faux déjà entouré. L'ensemble de la démonstration se tient sur papier sans rien perdre.",
      },
    },
    pratique: {
      consigne:
        "Cinq temps chronométrés. Premier temps : rédigez le commentaire de votre dernière campagne à partir de VOS chiffres saisis à la main — jamais de l'export — puis faites proposer trois hypothèses de test à budget constant et tranchez celle que vous retenez. Deuxième temps : vérifiez en direct comment votre marque est décrite par un assistant, avec les trois questions fournies (qui est cette entreprise, que vend-elle, à qui la recommanderiez-vous), relevez les erreurs et listez ce qui se corrige sur vos propres pages ; puis dégrossissez une veille sur deux concurrents et marquez d'une croix tout ce qui reste à vérifier à la source. Troisième temps : chasse à l'erreur, sur un texte produit en direct à propos de votre marché — chacun surligne ce qui est faux ou invérifiable, on compte à voix haute, et la salle en tire les quatre contrôles de sa propre grille de relecture. Quatrième temps : classez les douze cas de diffusion du kit en « je publie » / « je publie avec mention » / « je ne publie pas ». Cinquième temps : assemblez le document unique, daté et nominatif, et désignez qui le tient à jour et à quelle date il est revu.",
      aEmporter:
      ...
      notes: {
        script:
          "Répétez la règle au démarrage de l'atelier, pas seulement à 14 h : AUCUN export, AUCUN fichier de contacts. Les quatre valeurs se saisissent à la main, point. Sur la visibilité de marque : faites relever les erreurs PAR ÉCRIT, et faites écrire en face ce qui se corrige sur leurs propres pages — sans cette seconde colonne, l'atelier n'est qu'un constat vexant. Sur la veille : la croix « à vérifier à la source » n'est pas décorative, exigez-la sur chaque ligne. Sur la chasse à l'erreur : comptez les repérages À MAIN LEVÉE avant de corriger et notez le score au tableau — la salle sous-estime toujours, et le chiffre affiché vaut mieux qu'un discours sur la vigilance. Sur les douze cas : corrigez en salle avec le corrigé du kit, et sur toute question de droit qui déborde, la formule exacte se prononce telle quelle — « je ne me prononce pas, notez la question, votre conseil tranchera ». Sur le montage : faites NOMMER la personne et FIXER la date. Un document sans titulaire n'est pas tenu.",
        faq: [
          {
        ...
        blocages: [
          ...
          },
          {
            ...
            parade:
              "Coupez le cinquième temps à l'essentiel : le nom du fichier, le titulaire, la date de revue. Le reste s'assemble au bureau ; ces trois lignes-là, non.",
          },
        ],
        planB:
          "Outil indisponible : le commentaire de campagne s'écrit à la main sur la trame fournie et les trois hypothèses de test se listent en binôme — c'est un exercice d'arbitrage, pas de génération. La chasse à l'erreur se fait sur le texte imprimé du kit (page 29). Les douze cas de diffusion et le montage du livrable ne dépendent d'aucun outil. Pour la visibilité de marque, le kit contient trois réponses d'assistant pré-enregistrées sur des entreprises réelles (page 31) : l'exercice de relevé d'erreurs se tient à l'identique.",
      },
    },
    verification: {
      question:
        "Deux temps. D'abord un contrôle croisé en binôme sur la grille fournie : aucun chiffre calculé par l'outil ne subsiste-t-il dans le commentaire de campagne, aucune donnée client n'a-t-elle été déposée, chaque affirmation de veille porte-t-elle la mention vérifiée ou à vérifier ? Puis l'évaluation des acquis : quiz individuel de dix questions, corrigé en salle question par question, suivi de l'auto-évaluation d'une production du jour sur la grille de relecture construite par la salle.",
      reponseAttendue:
        "Le contrôle croisé se solde par au moins un écart relevé par binôme, annoté sur la copie du voisin. Le corrigé du quiz est commenté question par question, sans en passer aucune ; le seuil de réussite est celui déclaré au programme, et un score en dessous déclenche la reprise individuelle prévue au dispositif.",
      dureeMin: 25,
      notes: {
        script:
          "Le chiffre recalculé par l'outil est ce qui échappe le plus : il ne s'annonce pas, il se glisse dans une phrase de conclusion. Faites-le chercher avant de le nommer. Sur le quiz : c'est l'évaluation des acquis au sens de l'indicateur 11 — elle se tient, elle se corrige, elle se conserve. Ne la sacrifiez jamais au temps qui manque ; c'est la première chose qu'un auditeur demande. Si vous êtes en retard, coupez ailleurs.",
        faq: [
          {
        ...
        planB:
          "Grille de contrôle, quiz et corrigé sont imprimés dans le kit (pages 33 à 36). Aucune dépendance à un outil, ni à un vidéoprojecteur.",
      },
    },
    synthese: {
      acquis: [
