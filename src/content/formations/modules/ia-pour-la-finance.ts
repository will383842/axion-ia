/**
 * CONTENU RÉDIGÉ — « IA pour la finance » (1 jour, 7 h, 2 modules).
 *
 * ## Pourquoi DEUX modules et non quatre
 *
 * Le catalogue découpe cette journée en deux sections — « Matin » et
 * « Après-midi » — et non en quatre. Les libellés « Module 1 · », « Module 2 · »
 * qui apparaissent DANS les titres de séquences sont des repères de lecture pour
 * la fiche publique, pas des modules : ils n'ont pas d'identifiant, et rien ne
 * les rattache au programme. Ce qui porte un `moduleId` est la section. Le
 * contenu suit donc le catalogue à la lettre : `mod-1` couvre toute la matinée
 * (documents financiers, tableur assisté, trames de contrôle), `mod-2` toute
 * l'après-midi (écrire autour du chiffre, fiabilité, ancrage).
 *
 * ## Le fil rouge
 *
 * Chacun repart avec UN classeur de clôture assistée, monté pièce par pièce :
 * la matinée y verse la ligne de partage, une synthèse de document long, une
 * formule documentée et une trame de contrôle ; l'après-midi y ajoute le
 * commentaire de gestion, la séquence de relance, la règle de relecture du
 * service. L'ordre est l'enseignement : on pose ce qui ne sort pas avant de
...
export const IA_POUR_LA_FINANCE: EnrichissementFormation = [
  // ───────────────────────────────────────────────────────────────────────────
  // Module 1 (matin) — Le partage des rôles, le tableur assisté, les trames
  // ───────────────────────────────────────────────────────────────────────────
  {
    moduleId: "mod-1",
    objectif: {
      enonce:
        "À la fin de ce module, vous qualifiez un document financier dans son régime d'usage avant de l'ouvrir dans un outil, vous en tirez une synthèse dont chaque affirmation renvoie à une page, et vous obtenez une formule de tableur sans qu'aucune donnée ne quitte votre fichier.",
      objectifGlobalId: "obj-1",
      // La matinée pose la liste rouge et les trois régimes d'usage (obj-5), et
      // c'est elle qui traite la question du rapprochement — en la tranchant :
      // une trame de contrôle déroulable, pas un rapprochement automatique.
      objectifsSecondairesIds: ["obj-5", "obj-2"],
      dureeMin: 15,
      notes: {
        script:
          "Tour de table en UNE phrase, pas une par personne qui s'étale : « votre prénom, et le document qui vous prend le plus de temps à lire ». Notez chaque réponse au tableau, vous y reviendrez à 17 h. Écrivez ensuite au paperboard la phrase qui tient la journée et laissez-la affichée : les chiffres restent dans vos systèmes, l'IA travaille sur tout ce qui les entoure. Annoncez le livrable, c'est ce qui achète l'attention : chacun repart avec son classeur de clôture assistée. Au retour de la pause, cinq minutes suffisent pour ouvrir la partie tableur — annoncez que personne ne collera la moindre cellule dans l'outil, qu'on décrira seulement les en-têtes de colonnes, et laissez la salle réagir : c'est le point qui surprend le plus.",
        faq: [
          {
            question: "On va enfin pouvoir lui faire faire nos rapprochements ?",
            reponse:
              "Non, et c'est une décision, pas une limite qu'on subit. Un modèle de langage ne calcule pas, il rédige le calcul le plus probable — vous le constaterez vous-même dans une heure. Ce que vous obtiendrez à la place est une trame de contrôle qu'une autre personne peut dérouler, cocher et signer. C'est moins spectaculaire, et c'est opposable.",
          },
          {
    ...
    demonstration: {
      ...
      notes: {
        script:
          "Trois démonstrations s'enchaînent, ne les mélangez pas. (1) Lancez la demande spontanée sur le rapport du kit et LISEZ LA SORTIE À VOIX HAUTE avec conviction : la salle doit la trouver bonne avant qu'on l'abatte. Envoyez ensuite deux stagiaires chercher dans le document imprimé les deux montants annoncés — ils ne les trouveront pas. Ce moment-là tient toute la journée : ne le racontez pas, faites-le faire. Enchaînez sur la version cadrée. (2) Écrivez AXION au tableau et laissez-le affiché jusqu'au soir : Acteur, conteXte, Intention, Output, Normes. Tapez le prompt tableur devant eux et dites à voix haute, au moment de valider, que vous n'avez collé aucune cellule. (3) Projetez la formule héritée sans un mot pendant trente secondes, laissez le silence s'installer, puis lancez l'explication.",
        faq: [
          {
            question:
        ...
        planB:
          "Quota atteint, réseau tombé ou service indisponible : les trois sorties sont imprimées dans le kit formateur (pages 4 à 9, datées), montants inventés déjà entourés, et le rapport du commissaire aux comptes est fourni imprimé. La recherche des deux montants dans le document se tient à l'identique sur papier — c'est même là qu'elle est la plus frappante. N'improvisez aucune démonstration de remplacement en direct : les sorties du kit ont été vérifiées, les vôtres ne le seront pas.",
      },
    },
    pratique: {
      consigne:
        "Quatre temps chronométrés. (1) Faites-la se tromper : demandez à l'outil de lettrer les deux extraits du kit, de recalculer le total de la page 3 et de prévoir un atterrissage à partir des trois lignes fournies — relevez PAR ÉCRIT chaque erreur produite. (2) Qualifiez à la main, sur la grille, un document long de votre propre service, puis travaillez sur le document équivalent du kit : tirez-en une synthèse structurée et trois questions à poser à son émetteur. (3) Apportez un besoin de tableur réel : décrivez la structure de vos colonnes, sans aucune donnée, et repartez avec votre formule, votre croisement ou votre procédure — testés sur votre fichier et documentés en français dans le classeur. (4) Produisez votre trame de contrôle sur un processus réel — clôture, cut-off, états de frais — en recopiant EN TÊTE de trame la borne posée le matin : un écart signalé ne désigne jamais une personne.",
      aEmporter:
        "La ligne de partage écrite de votre main (ce que l'IA rédige, ce qu'elle ne calculera jamais), une synthèse de document long avec ses trois questions à l'émetteur, une formule ou un croisement testé et documenté en français, et votre trame de contrôle — première moitié du classeur de clôture assistée.",
      dureeMin: 95,
      notes: {
        script:
          "Annoncez le chronomètre à voix haute toutes les cinq minutes, sur les quatre temps. Temps 1 : ne corrigez RIEN pendant l'exercice, laissez les erreurs s'accumuler. Tracez ensuite deux colonnes au tableau — « elle rédige » et « elle ne calcule pas » — et faites-les remplir par la salle. La ligne de partage doit être écrite par eux, sinon elle n'est pas retenue. Temps 2 : dites-le au démarrage de l'atelier et pas seulement en ouverture de journée — on qualifie le sien, on dépose celui du kit. Temps 3 : circulez, la faute la plus fréquente est de coller trois lignes « juste pour donner un exemple ». Temps 4 : exigez la borne recopiée en tête de trame, pas en bas de page — en bas de page, personne ne la lit.",
        faq: [
          {
        ...
        planB:
          "Réseau ou outil indisponibles : les quatre temps se tiennent sur papier. Le temps 1 utilise les sorties fautives imprimées du kit (pages 10 à 12), le temps 2 la synthèse imprimée à corriger, le temps 3 s'écrit à la main — décrire une structure de colonnes est un exercice de rédaction, pas d'outil, et la formule se recopiera lundi. Le temps 4 ne dépend d'aucun outil : c'est le plus robuste de la matinée, gardez-le complet même si tout le reste a été dégradé.",
      },
    },
    verification: {
      question:
        "Contrôle croisé en binôme, grille fournie, sur les productions de l'autre : le régime d'usage retenu pour le document était-il le bon ? quelle affirmation de la synthèse ne figure pas dans le document source ? quel chiffre a été repris sans être vérifié à la page citée ? Puis sur la trame de contrôle : quelqu'un qui n'a pas assisté à la clôture peut-il la dérouler et la cocher seul, et la borne est-elle bien écrite en tête ?",
      reponseAttendue:
        "Chaque affirmation non rattachée à une page est BARRÉE sur la copie du binôme, chaque chiffre non vérifié entouré, et tout point de trame qui commence par « vérifier que tout est correct » est réécrit en geste vérifiable. Le corrigé projeté ensuite montre les trois formes que prend une affirmation non sourcée : le chiffre arrondi, la cause supposée, et la comparaison avec un exercice précédent qui n'a jamais été fourni.",
      dureeMin: 15,
      notes: {
        script:
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // Module 2 (après-midi) — Écrire autour du chiffre, fiabilité et ancrage
  // ───────────────────────────────────────────────────────────────────────────
  {
    moduleId: "mod-2",
    objectif: {
      enonce:
        "À la fin de ce module, vous faites dire à vos indicateurs déjà calculés ce qu'ils veulent dire, pour le lecteur que vous avez nommé, et vous repérez seul le moment où l'IA se trompe sur un chiffre.",
      objectifGlobalId: "obj-3",
      // La chasse à l'erreur, la grille de relecture et le quiz de fin de journée
      // installent le contrôle avant diffusion.
      objectifsSecondairesIds: ["obj-4"],
      dureeMin: 10,
      notes: {
        script:
          "Deux ouvertures dans ce module, cinq minutes chacune. À la reprise, posez la règle de l'après-midi et écrivez-la au tableau : les chiffres sont déjà calculés et déjà vérifiés par vous, l'IA ne fait que les mettre en phrase. Faites noter à chacun le NOM du lecteur de son prochain reporting — direction, opérationnels, banque, associé : sans destinataire nommé, l'atelier qui suit produit du texte interchangeable. Après la seconde pause, annoncez le dernier temps en une phrase : « on va regarder à quel moment elle se trompe sur un chiffre, et vous repartez avec le classeur monté ».",
        faq: [
          {
    ...
    demonstration: {
      ...
      notes: {
        ...
        planB:
          "Les trois séquences sont imprimées dans le kit (pages 20 à 23, datées) : commentaire libre et commentaire cadré, les trois trames, et les deux résultats divergents du même calcul avec l'écart déjà encadré. La comparaison se tient à l'identique sur papier, y compris le « d'où sort cette cause ? » posé à la salle.",
      },
    },
    pratique: {
      consigne:
        "Quatre temps chronométrés. (1) Rédigez le commentaire de votre dernier reporting à partir de vos indicateurs déjà calculés, saisis à la main — aucun fichier déposé, aucun nom de client, aucune donnée nominative — puis produisez la version destinée à un second lecteur que vous aurez nommé. (2) À partir des trames fournies, produisez soit votre séquence de relance d'impayé à trois niveaux, soit votre note de synthèse au dirigeant. (3) Chasse à l'erreur chronométrée sur la note financière du kit : quatre erreurs de chiffre et deux affirmations non sourcées y sont plantées — surlignez ce que vous croyez faux, on compte à main levée avant d'ouvrir le corrigé. (4) Montez et nommez votre classeur de clôture assistée : trames de contrôle, formules documentées, trame de commentaire, séquence de relance, liste rouge du service, glossaire maison — rangé pour être rouvert à la prochaine clôture.",
      aEmporter:
        "Le commentaire de votre dernier reporting en deux niveaux de lecture, une séquence de relance d'impayé à trois niveaux ou une note de synthèse au dirigeant, la règle de relecture du service écrite en une phrase, et le classeur de clôture assistée nommé, rangé, avec le nom de la personne qui le tient à jour.",
      dureeMin: 120,
      notes: {
        script:
          "Temps 1 : exigez le NOM du second lecteur avant la première ligne — un commentaire sans destinataire nommé revient toujours interchangeable. Circulez et cherchez la faute propre à cet atelier : le stagiaire qui laisse l'outil recalculer un pourcentage « juste pour vérifier ». Temps 2 : les mentions dues se recopient de la trame, jamais de l'outil, et l'en-tête « Projet — à faire valider par votre conseil avant diffusion » reste apparent. Vous n'arbitrez AUCUNE question juridique ni comptable ; la formule se prononce telle quelle — « je ne me prononce pas, notez la question, votre conseil tranchera » — et vous écrivez la question au tableau : la liste des questions ouvertes est une production utile de la journée. Temps 3 : comptez les repérages À MAIN LEVÉE et notez le score au tableau AVANT d'ouvrir le corrigé. La salle sous-estime toujours, et le chiffre affiché vaut mieux qu'un discours sur la vigilance. Faites ensuite écrire la règle de relecture du service en une phrase, par eux. Temps 4 : faites nommer le fichier ET la personne qui le tient — un classeur sans titulaire n'est jamais rouvert.",
        faq: [
          {
        ...
        planB:
          "Aucun outil n'est indispensable : les quatre temps se tiennent sur papier. Le temps 1 s'écrit à la main sur la trame de commentaire du kit — ce qui compte est la structure de la demande, pas l'outil qui l'exécute. Le temps 2 utilise les trames imprimées, qui sont de toute façon la source des mentions. Le temps 3 est déjà sur papier par construction. Le temps 4 est un travail de rangement : c'est l'atelier le plus robuste de la journée, gardez-le complet même si tout le reste a été dégradé.",
      },
    },
    verification: {
      question:
        "Deux temps. Contrôle croisé en binôme, grille fournie, sur les écrits de l'autre : un chiffre y figure-t-il sans avoir été vérifié à sa source ? le ton correspond-il au niveau de relance ? les mentions dues sont-elles présentes et reprises de la trame ? le destinataire et le niveau de lecture sont-ils cohérents ? Puis, individuellement, évaluation des acquis : quiz de dix questions corrigé en salle, suivi de l'auto-évaluation d'une production du jour sur la grille de relecture du classeur.",
      reponseAttendue:
        "Sur le contrôle croisé, les quatre points passés en revue sur chaque écrit, écarts relevés à l'oral. Sur le quiz, le corrigé est commenté question par question sans en sauter aucune ; le seuil de réussite est celui déclaré au programme, et un score en dessous déclenche la reprise individuelle prévue au dispositif.",
      dureeMin: 25,
      notes: {
        script:
          "C'est l'évaluation des acquis au sens de l'indicateur 11 : elle se tient, elle se corrige, elle se conserve. Ne la sacrifiez jamais au temps qui manque — si vous êtes en retard, coupez sur le contrôle croisé, jamais sur le quiz. C'est la première chose qu'un auditeur demande. Sur l'auto-évaluation, faites cocher la grille sur une production RÉELLE du jour, sortie du classeur, pas sur un souvenir de l'après-midi.",
        faq: [
          {
        ...
        planB:
          "Quiz papier dans le kit (page 27) et corrigé page 28, grille de contrôle croisé page 26. Aucune dépendance à un outil, et c'est délibéré : l'évaluation ne peut pas sauter parce que le réseau est tombé.",
      },
    },
    synthese: {
      acquis: [
