/**
 * CONTENU RÉDIGÉ — « IA pour la relation client » (1 jour, 7 h, 2 modules).
 *
 * ## Le fil rouge
 *
 ...
 * quatre contrôles. Le matin construit la matière, l'après-midi la rend
 * opposable et la range. Aucun des deux ne se suffit à lui-même.
 *
...
export const IA_POUR_LA_RELATION_CLIENT: EnrichissementFormation = [
  // ───────────────────────────────────────────────────────────────────────────
  // Module 1 — Matin : le cadre du ticket, puis les réponses
  // ───────────────────────────────────────────────────────────────────────────
  {
    moduleId: "mod-1",
    objectif: {
      enonce:
        "À la fin de ce module, vous produisez une réponse type réutilisable et une réclamation traitée, et vous neutralisez un ticket réel avant de le confier à un outil.",
      objectifGlobalId: "obj-1",
      // Le matin traite aussi une réclamation de bout en bout (atelier + chasse
      // à l'erreur), et installe la liste rouge et les trois régimes d'usage.
      objectifsSecondairesIds: ["obj-2", "obj-5"],
      dureeMin: 15,
      notes: {
        script:
          "La première question de la salle est toujours la même, et elle n'est jamais posée à voix haute : « est-ce qu'on va nous remplacer ? ». Posez-la vous-même, dès la deuxième minute, et répondez par le partage de tâches, pas par une promesse : l'IA propose un brouillon, le conseiller vérifie le fait, autorise l'engagement et signe. Tracez deux colonnes au tableau, « ce que la machine prépare » et « ce que je garde », et faites-les remplir en tour de table d'une phrase par personne. Gardez ce tableau affiché toute la journée. Annoncez enfin le livrable de la matinée : deux réponses types réutilisables et une réclamation traitée. Au retour de la pause, reprenez cinq minutes pour annoncer le module 2 : ce qu'on produit maintenant sera validé par un pair sur grille, pas par le formateur.",
        faq: [
          {
    ...
    demonstration: {
      avant:
        "Le même ticket traité comme aujourd'hui : le conseiller relit l'historique, retrouve le motif, cherche la réponse type la plus proche, la recolle et la retouche. Puis, sur une réclamation agressive, il écrit de zéro parce qu'aucune réponse type ne convient — et le ton dépend de son humeur du moment.",
      apres:
        "Le même ticket confié à un outil avec une consigne structurée par les cinq leviers AXION. Le brouillon arrive en une passe, dans le ton de la maison, avec les mentions obligatoires, et surtout SANS aucune garantie ni délai que personne n'a autorisés — parce que la consigne l'interdit explicitement.",
      prompt:
        "PROMPT 1, le ticket simple — « Voici la demande d'un client, reproduite ci-dessous. Rédige un brouillon de réponse. Réponds uniquement à partir de ce qui est écrit dans la demande et dans la fiche produit jointe. N'annonce aucun délai, aucun remboursement, aucun geste commercial et aucune garantie : si l'un d'eux te paraît nécessaire, écris à la place [À ARBITRER PAR LE CONSEILLER] et poursuis. Termine par la question qu'il me reste à poser au client pour clore le dossier. »\n\nPROMPT 2, la réclamation agressive, structuré par AXION —\nActeur : tu es conseiller au service client d'une entreprise de vente d'équipement professionnel.\nConteXte : le client écrit pour la deuxième fois au sujet d'une livraison incomplète, son message est agressif et met en cause la personne qui lui a répondu la première fois. Le message et l'historique neutralisés sont ci-dessous.\nIntention : produire un brouillon de réponse qui reconnaît le fait, ne discute pas le ton, et propose la prochaine étape concrète.\nOutput : 150 mots maximum, sans liste à puces, une seule question au client à la fin.\nNormes : vouvoiement, pas d'excuse formulée plus d'une fois, aucune mise en cause d'un collègue, aucun délai ni geste commercial chiffré — écris [À ARBITRER PAR LE CONSEILLER] à leur place. Ajoute en dernière ligne la mention d'information sur l'usage d'un outil d'assistance, telle qu'elle est fournie dans le kit.",
      outil: "Un seul outil, celui validé dans la salle (Claude ou ChatGPT selon le groupe).",
      gain: { avant: "18 min", apres: "5 min" },
      captureEcran:
        "Les deux brouillons côte à côte, avec les mentions [À ARBITRER PAR LE CONSEILLER] entourées, et les cinq leviers AXION annotés en marge du second prompt.",
      verifieLe: VERIFIE_LE,
      dureeMin: 35,
      notes: {
        script:
          "Chronométrez le traitement « comme aujourd'hui » en direct, montre en main, et annoncez le temps à voix haute : c'est le seul chiffre que la salle retiendra du matin. Écrivez AXION au tableau — Acteur, conteXte, Intention, Output, Normes — et laissez-le affiché jusqu'au soir. Sur la réclamation agressive, faites lire le message du client à voix haute par un stagiaire volontaire avant de lancer quoi que ce soit : le ton se ressent, il ne se décrit pas. Insistez sur la ligne Normes, et une seule fois, en la nommant : c'est elle qui empêche l'outil d'inventer un délai de remboursement. Montrez ensuite ce que le [À ARBITRER PAR LE CONSEILLER] produit dans le brouillon — c'est la trace visible de la décision qui reste humaine.",
        faq: [
          {
        ...
        blocages: [
          {
            ...
            parade:
              "Relancer le prompt 1 en retirant la phrase d'interdiction, et en ajoutant « rassure le client ». Le délai et le geste commercial réapparaissent presque toujours. Si ce n'est toujours pas le cas, passer au plan B sans commenter l'échec.",
          },
          {
            situation: "Un stagiaire propose de tester avec un ticket ouvert dans son outil.",
            parade:
              "Refuser en une phrase : « aucune donnée réelle de client aujourd'hui, et l'atelier de 11 h vous apprend précisément à préparer vos vrais tickets pour ça ».",
          },
        ],
        planB:
          "Quota atteint, réseau tombé ou réponse aberrante : les quatre sorties sont imprimées dans le kit formateur (pages 5 à 8, datées). La comparaison se tient à l'identique sur papier, y compris le chronométrage du traitement « comme aujourd'hui », qui ne dépend d'aucun outil. Ne rien improviser en direct : les sorties du kit ont été vérifiées.",
      },
    },
    pratique: {
      consigne:
        "Deux temps. D'abord, constituez VOTRE jeu de travail de la journée : prenez trois demandes réellement reçues — une simple, une réclamation, une hors périmètre — et neutralisez-les sur la trame fournie, en appliquant la liste rouge (nom, adresse, numéro de dossier, pièce d'identité, moyen de paiement, et tout détail qui permettrait de reconnaître la personne). Ensuite, traitez ces trois demandes avec la méthode AXION, puis faites contrôler vos trois réponses par votre binôme sur la grille fournie : exactitude du fait, engagement pris, ton, réutilisabilité.",
      aEmporter:
        "Le jeu de trois demandes neutralisées, les trois réponses correspondantes dont deux réponses types réutilisables, et la fiche mémo des cinq leviers AXION — première pièce de votre référentiel.",
      dureeMin: 60,
      notes: {
        script:
          "Sur la neutralisation, ne laissez personne se dispenser d'écrire à la main : c'est le geste qui compte, pas le résultat. Passez dans les rangs et cherchez UNE chose, toujours la même — le détail qui reste identifiant après le retrait du nom. « Le client de la mairie de Voiron qui a commandé trois palettes en janvier » est encore identifiable, et le stagiaire ne le voit pas seul. Montrez-le une fois à voix haute, la salle corrige ses propres copies dans la minute. Sur l'atelier de réponses : les trois consignes sont écrites et remises, ne les dictez pas. Rappelez que la réponse hors périmètre est la plus difficile — c'est celle où l'on doit dire non sans fermer la porte. Le contrôle croisé se fait sur la grille, au stylo, sur la copie de l'autre.",
        faq: [
          {
        ...
        planB:
          "L'atelier de neutralisation ne dépend d'aucun outil : il s'écrit à la main sur la trame fournie. Si l'outil est indisponible pour la seconde partie, les stagiaires rédigent leur consigne AXION en entier sur le support — c'est la consigne qui s'apprend, pas la manipulation — et le contrôle croisé porte sur les consignes plutôt que sur les réponses. Chacun relancera chez lui : la trame est datée et réutilisable.",
      },
    },
    verification: {
      question:
        "Trois contrôles corrigés en salle. Premier : chacun classe l'outil réellement utilisé par son équipe dans l'un des trois régimes d'usage, et écrit en une phrase ce qu'il a le droit d'y coller. Deuxième, cinq questions sur les cinq extraits fournis : qu'est-ce qui peut être collé tel quel, qu'est-ce qui doit être neutralisé, qu'est-ce qui ne sort jamais ? Troisième, la chasse à l'erreur : dans les réponses produites à l'atelier, repérez la garantie, le délai ou le geste commercial que personne n'a autorisés — on compte, chaque binôme annonce son score.",
      reponseAttendue:
        "L'outil de chaque stagiaire est nommé et classé, avec sa phrase de périmètre. Les cinq extraits sont classés conformément au corrigé — l'extrait 3, un historique où le nom a été retiré mais dont la commune et le numéro de commande subsistent, est celui qui se rate le plus. Sur la chasse à l'erreur, chaque binôme annonce son score à voix haute et le total de la salle est écrit au tableau.",
      dureeMin: 38,
      notes: {
        script:
          "Sur le classement des régimes : faites écrire le NOM de l'outil, pas sa catégorie. « Un compte gratuit ouvert avec l'adresse professionnelle » est une réponse, « une IA » n'en est pas une. Si personne ne sait ce que l'entreprise a validé, c'est déjà un résultat utile : notez-le comme question ouverte au tableau, elle partira dans la feuille de route de 17 h. Sur la chasse à l'erreur, comptez AVANT de corriger, et écrivez le total au tableau. Le chiffre affiché vaut mieux que n'importe quel discours sur la vigilance : la salle sous-estime toujours. Corrigez ensuite les deux erreurs les plus fréquentes seulement, le reste se lit sur le corrigé.",
        faq: [
          {
        ...
        planB:
          "Les cinq extraits, le corrigé et le jeu de réponses pour la chasse à l'erreur sont imprimés dans le kit (pages 14 à 17). Sans vidéoprojecteur, le corrigé se distribue et se commente à l'oral, et les scores s'écrivent au paperboard.",
      },
    },
    synthese: {
      acquis: [
        ...
        blocages: [
          {
            ...
            parade:
              "Ne pas insister : énoncer les trois acquis vous-même, faire nommer le fichier, libérer. Un acquis récité à contrecœur ne s'ancre pas mieux.",
          },
        ],
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // Module 2 — Après-midi : dossiers en cours, base de connaissances, envoi
  // ───────────────────────────────────────────────────────────────────────────
  {
    moduleId: "mod-2",
    objectif: {
      enonce:
        "À la fin de ce module, vous reprenez un dossier en cours en cinq minutes à partir de son historique, et vous transformez vos réponses du matin en fiches opposables relues avant envoi.",
      objectifGlobalId: "obj-3",
      // L'après-midi produit aussi les fiches de base de connaissances, conduit
      // une réclamation de troisième relance jusqu'à l'escalade, et applique les
      // quatre contrôles avant envoi.
      objectifsSecondairesIds: ["obj-4", "obj-2", "obj-5"],
      dureeMin: 5,
      notes: {
        script:
          "Reprenez le tableau des deux colonnes tracé le matin, et montrez ce que la matinée a déplacé de la colonne droite vers la gauche. Annoncez les trois livrables de l'après-midi sans les commenter : une fiche de reprise de dossier, trois fiches de base de connaissances, et une réponse passée aux quatre contrôles. Dites aussi ce que l'après-midi ne fera pas — on ne branchera rien sur votre outil de ticketing aujourd'hui, et on ne publiera aucune réponse. L'honnêteté sur ce point achète la crédibilité du reste.",
        faq: [],
        blocages: [],
        planB: "Aucun outil en jeu.",
      },
    ...
    demonstration: {
      ...
      notes: {
        ...
        blocages: [
          ...
          {
            situation: "L'outil réécrit le brouillon au lieu de relever, sur le prompt 2.",
            parade:
        ...
        planB:
          "Les trois sorties sont imprimées dans le kit (pages 21 à 23, datées), avec les renvois de numéros déjà surlignés. Le pari écrit et le comptage des engagements se tiennent à l'identique sur papier.",
      },
    },
    pratique: {
      consigne:
        "Cinq temps, tous sur du matériel neutralisé. Un : reprenez un dossier en cours neutralisé et produisez sa fiche de reprise en trois blocs ; votre binôme vérifie qu'aucun engagement n'a été perdu ni inventé. Deux : jeu de rôle, l'outil joue le client mécontent qui revient une troisième fois — conduisez l'échange sur l'un des trois scénarios du kit, puis rédigez l'escalade vers le niveau 2 sur la trame fournie. Trois : regroupez le jeu de deux cents verbatims neutralisés en causes racines, et écrivez la note d'une page au responsable. Quatre : rédigez vos trois premières fiches de base de connaissances à partir de vos réponses du matin — un motif et un seul, une réponse validée, une date de revue, un propriétaire nommé. Cinq : appliquez les quatre contrôles à vos fiches et à votre réponse la plus fréquente, et posez-y la mention d'information du client vue le matin.",
      aEmporter:
        "Une fiche de reprise de dossier, une escalade rédigée, la note d'une page sur les causes racines, trois fiches de base de connaissances datées et attribuées, et votre réponse la plus fréquente passée aux quatre contrôles — le référentiel complet, rangé et nommé.",
      dureeMin: 125,
      notes: {
        script:
          "Sur le jeu de rôle, la consigne donnée à l'outil est fournie au kit et se colle telle quelle : ne la laissez pas improviser, un client simulé trop poli n'apprend rien et un client simulé insultant fait sortir un stagiaire de la salle. Annoncez le cadre avant de lancer : c'est un exercice, on l'arrête quand on veut, et personne ne commente la performance d'un autre. Sur les verbatims, imposez la contrainte qui fait tout le travail : cinq causes racines maximum. Sans plafond, la salle produit trente catégories et n'en tire rien. Sur les fiches de base de connaissances, tenez la règle du motif unique — une fiche qui traite deux motifs ne sera jamais retrouvée par la bonne recherche — et faites écrire le NOM d'un propriétaire, pas « le service ». Une fiche sans titulaire n'est pas maintenue. Sur le dernier temps, faites appliquer les quatre contrôles dans l'ordre, à voix basse, en cochant : c'est la répétition qui installe le réflexe. Rappelez enfin la règle de la journée à chaque démarrage d'atelier, pas seulement le matin : aucune donnée réelle de client ne part dans l'outil.",
        faq: [
          {
        ...
        planB:
          "Outil indisponible : le jeu de rôle se tient à deux, un stagiaire jouant le client à partir du scénario imprimé — c'est même la version la plus riche, gardez-la en réserve. Les verbatims se regroupent à la main sur les fiches cartonnées du kit, la note d'une page s'écrit au stylo, et les fiches de base de connaissances se rédigent sur la trame papier. C'est l'après-midi la plus robuste de la journée : aucun des cinq temps ne dépend réellement d'un accès IA, seule la vitesse change.",
      },
    },
    verification: {
      question:
