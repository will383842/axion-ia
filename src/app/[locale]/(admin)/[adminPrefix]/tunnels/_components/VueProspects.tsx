// Corps visuel du tunnel de prospects.
//
// Séparé de la page pour la même raison que `VueEnsemble` : sans session, la
// page ne s'affiche pas, donc son rendu ne pouvait être contrôlé à aucune
// largeur d'écran. Ce composant ne dépend que de ses propriétés.
  ...
  const colonnesAbandon: ReadonlyArray<AdminTableColumn<AbandonEcran>> = [
    {
      key: "rang",
      header: "Écran",
      // Libellé lisible, pas l'identifiant technique : « volume:ventes »
      // était exact et illisible.
      cell: (r) => `${r.stepIndex}. ${libelleEcran(r.step)}`,
      width: "40%",
    },
    ...
    <>
      ...
      {tunnel !== null ? (
        <p className="admin-alert admin-alert-info">
          <span>
            Vue restreinte à <strong>{LIBELLES[tunnel] ?? tunnel}</strong> : seules les balises
            émises sur cette page sont comptées. Un visiteur venu de la page publicitaire y apparaît
            sans son étape d&apos;arrivée.
          </span>
        </p>
      ) : null}
      ...
      <div className="admin-kpi-grid">
        ...
        <AdminStatCard
          ...
          value={pireEcran ? libelleEcran(pireEcran.step) : "—"}
          meta={
            pireEcran
              ? `Écran ${pireEcran.stepIndex} · ${nb(
                  pireEcran.atteintes - pireEcran.poursuivies,
                )} sessions perdues ici`
