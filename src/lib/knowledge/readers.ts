...
export interface FaqItem {
  ...
  // ── Enrichissement éditorial (2026-08-10) ────────────────────────────────
  // Tous optionnels : seules les entrées retravaillées les portent, et la fiche
  // `/faq/[slug]` ne rend une section que si sa donnée existe. Les Q/R générées
  // (Track B) et les entrées historiques restent donc strictement inchangées.
  /** Points scannables sous la réponse. */
  readonly keyPointsFr?: readonly string[];
  /** Chiffres-clés de la réponse. */
  readonly factsFr?: readonly { figure: string; label: string }[];
  /** Déroulé concret, quand la réponse en décrit un. */
  readonly stepsFr?: readonly { title: string; detail: string }[];
  /** Confusions à lever — ce que la réponse n'est PAS. */
  readonly nuancesFr?: readonly { title: string; detail: string }[];
  /**
   * Date de révision PROPRE à l'entrée (ISO). Sans elle, la fiche retombe sur
   * la constante globale, qui donnait la même date aux 88 pages.
   */
  readonly reviewedAt?: string;
  /** Questions réellement liées (ids), sinon repli sur la catégorie. */
...
export async function listFaqs(): Promise<readonly FaqItem[]> {
  ...
  if (!isKbBackendUnifiedFor("faq")) {
    ...
    const legacy: FaqItem[] = FAQ_GLOBAL.map((f) => ({
      ...
      // Champs d'enrichissement — absents pour les entrées non retravaillées.
      ...(f.fr.keyPoints ? { keyPointsFr: f.fr.keyPoints } : {}),
      ...(f.fr.facts ? { factsFr: f.fr.facts } : {}),
      ...(f.fr.steps ? { stepsFr: f.fr.steps } : {}),
      ...(f.fr.nuances ? { nuancesFr: f.fr.nuances } : {}),
      ...(f.reviewedAt ? { reviewedAt: f.reviewedAt } : {}),
