...
interface Payload {
  ...
  /** Sprint 15 fix Fork 2 W4-2 : ajoute pour coherence avec booking-confirmed. */
...
const COPY = {
  fr: {
    ...
    body: (c: string, d: string, i: string, p?: number) =>
      `Votre option pour le ${d} (${i}${p ? `, ${p} participant${p > 1 ? "s" : ""}` : ""}) au nom de ${c} est enregistrée. Notre équipe valide ou refuse sous 48h.`,
  ...
  en: {
    ...
    body: (c: string, d: string, i: string, p?: number) =>
      `Your option for ${d} (${i}${p ? `, ${p} participant${p > 1 ? "s" : ""}` : ""}) on behalf of ${c} is recorded. Our team validates or refuses within 48 hours.`,
  ...
  return (
    ...
    >
      ...
      <Text style={emailStyles.paragraphStyle}>
        {t.body(p.companyName, p.bookingDate, p.interventionType, p.participantsCount)}
      </Text>
