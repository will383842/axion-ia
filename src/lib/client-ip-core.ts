// Cœur PUR de la résolution d'IP client — aucun import Next : utilisable en
// runtime edge (`app/api/vitals`). `client-ip.ts` le réexporte et ajoute
// `getClientIp()`, qui a besoin de `next/headers`. (Scindé le 2026-09-25.)
//
...
/**
 * Comme `ipDepuisEntetes`, mais rend `null` au lieu de « unknown ».
 *
 * 🔴 Pour les appelants qui HACHENT l'IP (`hashIp`) ou en font une clé de limite :
 * `hashIp(null)` rend `null`, alors que `hashIp("unknown")` rendrait une
 * empreinte réelle — la même pour TOUS les visiteurs sans IP. Une preuve de
 * signature porterait alors une fausse empreinte d'IP, et une limite
 * « par IP » deviendrait une limite commune à tous ces visiteurs.
 */
export function ipVisiteurOuNull(h: Pick<Headers, "get">): string | null {
  const ip = ipDepuisEntetes(h);
