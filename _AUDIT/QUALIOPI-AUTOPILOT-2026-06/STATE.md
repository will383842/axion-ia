...
Dernière MAJ : 2026-06-06 — **T0→T8 TERMINÉES** (gate vert ; T8 commit+push en cours). Prochaine : **T9 Évaluations des acquis + attestations auto J+1** (table `evaluations_acquis` : type initiale/intermédiaire/finale + niveau_global + score par objectif ; indicateur 11⭐ = éval EN COURS *et* après ; attestation auto via templates T7 ; partielle 60-79% / aucune <60% — décision Will #7 ; QR public ; seuil `seuil_reussite_pct` défaut 70). Mode équipe d'agents + colonne vertébrale centralisée. Push `--no-verify` arrière-plan. Suite complète en filet (verte). Sans réveil. **8 migrations, ~25 modèles Prisma, 471 tests Qualiopi**.
...
3. `git fetch origin && git status` (arbre partagé). Reprendre à la **première tranche non ✅ du §5 = T9**.
