...
Dernière MAJ : 2026-06-06 — Phase 0/0.5 + **T0 + T1 + T2 TERMINÉES** (gate vert, poussées main). Prochaine : T3 (modèles cœur Formation/TrainingSession/Trainer/Trainee/Enrollment + lien Booking + fiche publique `/formations/[slug]`). Mode : **équipe d'agents** (délégation écriture code + fan-out artefacts indépendants ; colonne vertébrale schéma/migration/gate/commit/push centralisée chez moi). Enchaînement continu, sans réveil.
