...
Dernière MAJ : 2026-06-06 — **T0→T6 TERMINÉES** (gate vert ; T0→T5 poussées main + suite complète 15223 verts à T4). Prochaine : **T7 Documents légaux PDF** (~14 templates React-PDF : convention+tripartite, convocation, émargement, relevé connexion, positionnement, grilles éval, satisfaction, attestation+partielle, certificat réalisation centièmes, factures, kits OPCO/CPF/FT, lettre mission) — modèle `DocumentGenere` + infra PDF partagée (fonts, base layout, render→S3→signed URL, SHA-256, QR, filigrane COPIE, suppression_prevue_at=+5ans) → **fan-out massif d'agents**. Prochaine : **T6 Sessions & inscriptions** (machine à états déjà posée T3 ; reste auto-transitions cron J-7/J-5/J±0/J+1/J+30 via repeatable jobs BullMQ miroir booking-crons, récurrentes max 52, report). Mode équipe d'agents + colonne vertébrale centralisée. Push `--no-verify` arrière-plan. Suite complète en filet (relancée après T4, verte). Sans réveil.
