...
**Doublons exacts** : 4
**Doublons sémantiques (Jaccard ≥75%)** : 14
**Keywords faibles** : 8
...
- Invalides : 135
  ...
  - `conférencier IA pour école de commerce privée` : metaDescription trop longue: 156 chars > 155
  - `automatiser la conception de formations avec l'IA` : metaDescription trop longue: 158 chars > 155
  - `formation IA sur site France` : metaDescription trop longue: 157 chars > 155
  - `formation IA pour PME industrielle manufacturière` : metaDescription trop longue: 157 chars > 155
...
4. `meilleur cabinet IA France 2026` ↔ `meilleur cabinet IA France classement 2026` (sim: 80%)
5. `meilleur cabinet IA France 2026` ↔ `meilleur cabinet IA France 2026 comparatif` (sim: 80%)
6. `quelle différence entre audit IA et diagnostic IA ?` ↔ `quelle différence entre audit IA PME et audit IA ETI ?` (sim: 80%)
7. `quelle différence entre audit IA PME et audit IA ETI ?` ↔ `quelle différence entre formation IA et audit IA ?` (sim: 80%)
8. `cabinet IA audit petite entreprise` ↔ `ROI audit IA petite entreprise` (sim: 75%)
9. `cabinet IA ETI France programme transformation` ↔ `programme IA transformation ETI France` (sim: 75%)
10. `formation IA pour équipes de direction ETI` ↔ `formation IA pour DSI et équipes IT en ETI` (sim: 75%)
11. `Axion-IA avis clients` ↔ `Axion-IA avis et références clients PME` (sim: 75%)
12. `combien d'heures faut-il pour former une équipe à l'IA ?` ↔ `combien de temps faut-il pour former une équipe à l'IA ?` (sim: 75%)
13. `quelle différence entre audit IA PME et audit IA ETI ?` ↔ `audit IA vs POC IA quelle différence` (sim: 75%)
14. `quelle différence entre audit IA PME et audit IA ETI ?` ↔ `audit IA vs POC IA quelle différence` (sim: 75%)
...
4. `est-ce qu'une ETI peut exiger des garanties de résultats à un cabinet IA ?` — trop long (>12 mots)
5. `la formation IA peut-elle être financée pour une TPE de moins de 10 salariés ?` — trop long (>12 mots)
6. `pourquoi une école privée doit former ses étudiants à l'IA dès 2026 ?` — trop long (>12 mots)
7. `quel est le coût d'une formation IA pour les formateurs d'un OF ?` — trop long (>12 mots)
8. `comment un CEO peut se former à l'IA sans y passer des heures ?` — trop long (>12 mots)
...
- prestataire audit IA micro-entreprise
...
- AI Act impact PME France conformité 2026
- Axion-IA est-il conforme RGPD et AI Act pour ses interventions
...
- comment détecter les biais dans un algorithme IA ?
...
- site ia performance e-commerce
...
- ROI audit IA petite entreprise
...
- agence IA formation équipe petite entreprise
...
- audit qualité prompts LLM entreprise
...
- quelle formation IA pour les dirigeants de PME ?
- coaching ia dirigeants
...
- consultant IA personnel CEO monthly coaching
...
- chatbot IA vs assistant IA différence pour PME
- chatbot RH IA
...
- developpement ia rag retrieval augmented
...
- déploiement agent IA entreprise sur mesure
...
- réduire la masse salariale indirecte ETI par l'automatisation IA
...
- automatiser relances clients IA CRM entreprise
...
- coaching ia prise de conscience ia enterprise
- ia integration erp crm site web
...
- site web ia json-ld schema markup seo
- refonte site web ia performance seo
...
- agence AEO France spécialisée PME
...
- définition GEO optimisation moteurs génératifs 2026
- SEO vs AEO vs GEO comparatif stratégies 2026
...
4. **Augmenter ai_overview** — actuellement 46 seeds → cible 8%
5. **Augmenter featured_snippet** — actuellement 36 seeds → cible 7%
6. **Équilibrer longue traîne** — 99% actuellement → cible 70%
...
9. **Supprimer keywords 1 mot** — 0 identifiés
